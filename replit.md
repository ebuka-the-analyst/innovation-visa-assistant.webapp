...
A **90-day SEO content automation system** (`server/seoAutomation.ts`) extracts all blog/keyword items from a generated SEO strategy and automatically queues them into the blog pipeline at 2-3 posts per week across 13 weeks. Plans are stored in `seo_automation_plans`, with a weekly Monday 8am GMT cron that advances to the next week. The admin SEO Strategy page (`client/src/pages/admin/SeoStrategy.tsx`) shows an inline automation panel with Activate / Pause / Resume / Replace Plan controls and a live progress bar. Routes: `POST /api/seo/activate-automation`, `GET /api/seo/automation-status`, `POST /api/seo/automation-toggle` (all admin-only).
