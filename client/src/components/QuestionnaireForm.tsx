...
export default function QuestionnaireForm({ tier = 'premium' }: { tier?: string }) {
  ...
  const handleEbukaUltimatePlanAutoFill = () => {
    const ebukaData: Record<string, string> = {
      ...
      productStatus: "MVP COMPLETE - Live production platform deployed at innovatorfoundervisaassistant.co.uk. 109 professional-grade AI-powered tools across 8 categories (Compliance, Documentation, Team, Business, Financial, Growth, Innovation, Defense). Technology stack: React 18, TypeScript, TailwindCSS, Node.js, Express, PostgreSQL (Neon), OpenAI GPT-4, Google Gemini integration. Features include: real-time compliance scoring, automated document generation, AI document review, pitch practice coach, financial projections calculator, innovation score assessment, interview preparation system. Full tier-based access control (Free/Basic/Premium/Enterprise/Ultimate). Mobile responsive, dark mode support, export to PDF/Word, QR code transfer, auto-save functionality. GitHub repository: github.com/bhenmedia/visa-assistant (private). Live demo available for endorser review.",
