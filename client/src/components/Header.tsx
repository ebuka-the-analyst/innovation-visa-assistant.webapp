...
import { INNOVATOR_FOUNDER_BASE_PATH, innovatorFounderPath } from "@/lib/innovator-founder-routes";
...
export default function Header() {
  ...
  const handleLogout = () => {
    ...
    queryClient.clear();
    setLocation(INNOVATOR_FOUNDER_BASE_PATH);
  ...
  const handleNavigation = (sectionId: string) => {
    ...
    const isUkLanding = location === "/" || location === INNOVATOR_FOUNDER_BASE_PATH;
    ...
    setLocation(`${INNOVATOR_FOUNDER_BASE_PATH}#${sectionId}`);
  };
    ...
    <header className="sticky top-0 z-50 w-full bg-background/80 backdrop-blur-xl">
      {!disclaimerDismissed && (
        ...
        >
          <div className="mx-auto max-w-7xl px-7 text-center text-xs leading-5 text-white sm:px-10 sm:text-sm">
            ...
            <a href={innovatorFounderPath("/ai-transparency")} className="underline underline-offset-2 hover:opacity-80">Learn more</a>
          </div>
          <button
      ...
      <nav className={`container mx-auto flex items-center justify-between border-b border-border/40 px-3 transition-[height] duration-200 ease-out md:px-6 ${isScrolled ? "h-14 md:h-16" : "h-20 md:h-24"}`}>
        <div className="isolate z-[9999] bg-transparent" data-testid="brand-logo">
          <VisaAssistantBrand routeFlag="🇬🇧" routeLabel="UK Innovator Founder" compact={isScrolled} logoHref="/" />
        </div>

        <div className={`hidden items-center transition-all duration-300 md:flex ${isScrolled ? "gap-4" : "gap-8"}`}>
          <Link href={innovatorFounderPath("/features")} className={`font-medium transition-colors hover:text-primary ${isScrolled ? "text-xs" : "text-sm"}`} data-testid="link-all-features">All Features</Link>
          <Link href={innovatorFounderPath("/tools-hub")} className={`font-medium transition-colors hover:text-primary ${isScrolled ? "text-xs" : "text-sm"}`} data-testid="link-tools-hub">Tools</Link>
          <Link href={innovatorFounderPath("/blog")} className={`font-medium transition-colors hover:text-primary ${isScrolled ? "text-xs" : "text-sm"}`} data-testid="link-blog">Blog</Link>
        ...
        <div className={`hidden items-center transition-all duration-300 md:flex ${isScrolled ? "gap-1" : "gap-2"}`}>
          ...
          {isAuthenticated ? (
            <>
              <Link href={innovatorFounderPath("/dashboard")}>
            ...
            <>
              <Link href={innovatorFounderPath("/login")}><Button variant="ghost" size={isScrolled ? "sm" : "default"} data-testid="button-header-signin">Sign In</Button></Link>
              <Link href={innovatorFounderPath("/pricing")}><Button size={isScrolled ? "sm" : "default"} data-testid="button-header-cta">Get Started</Button></Link>
            </>
          )}
        </div>
