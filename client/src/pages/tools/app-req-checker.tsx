    ...
    <>
      ...
      <div className="min-h-screen bg-gradient-to-br from-background via-accent/5 to-primary/5 p-6">
        ...
        <div className="max-w-5xl mx-auto">
          <h1 className="text-4xl font-bold mb-2">App Req Checker</h1>
          <p className="text-muted-foreground mb-6">70-point scoring system validation</p>
          ...
          <div className="grid md:grid-cols-4 gap-4 mb-6">
            ...
            <Card className="p-4">
              ...
              <p className="text-3xl font-bold mt-2">{done}/8</p>
            </Card>
            <Card className="p-4">
              <p className="text-xs">Status</p>
              <p className={`text-lg font-bold mt-2 ${score >= 80 ? "text-green-600" : score >= 60 ? "text-yellow-600" : "text-blue-600"}`}>
                {score >= 80 ? "✓ Ready" : score >= 60 ? "⚠ Nearly" : "✗ WIP"}
              </p>
            </Card>
            <Card className={`p-4 ${score >= 80 ? "bg-green-50" : "bg-blue-50"}`}>
              <p className="text-xs">Critical</p>
              <p className="text-2xl font-bold mt-2">{ITEMS.filter(i => checks[i.id] && i.pen === "Critical").length}/3</p>
            </Card>
          </div>
          ...
          <Card className="p-4 mb-4">
            ...
            <div className="w-full bg-gray-200 h-3 rounded-full">
              <div style={{ width: `${score}%` }} className="bg-primary h-3 rounded-full" />
            </div>
          </Card>

          <div className="space-y-2">
            {ITEMS.map(i => (
              <Card key={i.id} className={`p-3 border-l-4 cursor-pointer ${i.pen === "Critical" ? "border-l-red-500 bg-red-50" : "border-l-blue-500 bg-blue-50"}`} onClick={() => setExpanded({ ...expanded, [i.id]: !expanded[i.id] })}>
                <div className="flex gap-3">
                  <Checkbox checked={checks[i.id] || false} onCheckedChange={() => setChecks({ ...checks, [i.id]: !checks[i.id] })} onClick={e => e.stopPropagation()} />
                ...
                {expanded[i.id] && (
                  <Card className="mt-2 p-2 bg-white text-xs">
                    <p className="font-semibold mb-1">Info: {i.guide}</p>
                    <p className="text-blue-600">Action: {i.action}</p>
                  </Card>
                )}
              </Card>
            ))}
