...
export default function ComplianceXRay() {
  ...
  const saveProgress = () => {
    ...
    localStorage.setItem('complianceXRayDate', new Date().toLocaleDateString());
  ...
  const loadProgress = () => {
    ...
    if (saved) {
      ...
      const date = localStorage.getItem('complianceXRayDate');
      setSavedDate(date || '');
    }
  };
  ...
  useEffect(() => {
    const handoffKey = 'compliance-xray_handoff';
