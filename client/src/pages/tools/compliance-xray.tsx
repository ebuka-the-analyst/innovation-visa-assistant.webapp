    ...
    <>
      ...
      <div className="min-h-screen bg-gradient-to-br from-background via-accent/5 to-primary/5 p-6">
        ...
        <div className="max-w-5xl mx-auto">
          ...
          <p className="text-muted-foreground mb-6">Deep compliance analysis</p>
          ...
          <div className="flex gap-2 mb-6 flex-wrap">
            ...
            <Button onClick={() => {}} className="gap-2" variant="outline"><Download className="w-4 h-4" /> Export</Button>
          ...
          <Tabs value={tab} onValueChange={setTab} className="mb-6">
            ...
            <TabsContent value="overview" className="space-y-4">
              <Card className="p-4 mb-4">
                <div className="flex justify-between"><span className="font-bold">Progress</span><span>{score}%</span></div>
                <div className="w-full bg-gray-200 h-2 rounded-full mt-2"><div style={{width:`${score}%`}} className="bg-primary h-2 rounded-full"/></div>
              </Card>
            </TabsContent>

            <TabsContent value="analysis" className="space-y-2">
              {ITEMS.map(i=><Card key={i.id} className="p-3">
                <label className="flex gap-3"><Checkbox checked={checks[i.id]||false} onCheckedChange={()=>setChecks({...checks,[i.id]:!checks[i.id]})}/>
                  <div><p className="text-sm font-semibold">{i.name}</p></div></label></Card>)}
            </TabsContent>
          </Tabs>
        </div>
      </div>
