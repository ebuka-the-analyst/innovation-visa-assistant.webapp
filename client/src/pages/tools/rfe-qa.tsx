...
const AI_TOOL_CONFIG: ToolConfig = {
  ...
  agent: 'sage',
  greeting: "Hello! I'm Sage, your Compliance Expert. Let's systematically work through each RFE question from the Home Office. I'll help you craft comprehensive responses with the right evidence to address their concerns effectively.",
  questions: [
    {
      id: 'rfe-date',
      question: "When did you receive the RFE and what is the response deadline?",
      hint: "Home Office typically allows 28 days. Include the exact dates.",
      fieldKey: 'rfeDate',
      minLength: 10
    },
    {
      id: 'question-count',
      question: "How many specific questions or points does the RFE contain?",
      hint: "Count each distinct question or clarification request.",
      fieldKey: 'questionCount',
      fieldType: 'number'
    },
    {
      id: 'primary-questions',
      question: "What are the 2-3 most critical questions in your RFE that require immediate attention?",
      hint: "Focus on questions about core visa criteria: innovation, viability, or scalability.",
      fieldKey: 'primaryQuestions',
      minLength: 50
    },
    {
      id: 'evidence-gaps',
      question: "What evidence gaps have been identified? What documentation is specifically requested?",
      hint: "List missing documents like bank statements, technical assessments, customer letters, etc.",
      fieldKey: 'evidenceGaps',
      minLength: 30
    },
    {
      id: 'endorser-contact',
      question: "Have you contacted your endorsing body about the RFE? What was their guidance?",
      hint: "Endorsing bodies often provide support letters or clarification for RFE responses.",
      fieldKey: 'endorserContact',
      minLength: 20
    },
    {
      id: 'response-plan',
      question: "What is your timeline and plan for gathering the required evidence?",
      hint: "Break down the remaining time into specific tasks for each RFE question.",
      fieldKey: 'responsePlan',
      minLength: 30
    }
  ],
  completionMessage: "Great work! I've mapped out your RFE questions and response requirements. I'm now populating your Q&A tracker with each question, required evidence, and suggested response strategies."
};
...
export default function RFEQADefense() {
  ...
  const [mode, setMode] = useState<'ai' | 'traditional'>(() => {
    const saved = localStorage.getItem('rfe-qa-mode');
  ...
  useEffect(() => {
    if (!isPaidUser && mode === 'ai') {
      setMode('traditional');
      localStorage.setItem('rfe-qa-mode', 'traditional');
    }
  }, [isPaidUser, mode]);

  useEffect(() => {
    localStorage.setItem('rfe-qa-mode', mode);
  }, [mode]);

  const handleAiComplete = (answers: Record<string, string>) => {
    if (answers.rfeDate) setResponseDeadlineDate(answers.rfeDate);
    if (answers.primaryQuestions) {
      setQuestions([{
        id: Date.now().toString(),
        category: 'other',
        question: answers.primaryQuestions,
        yourResponse: '',
        evidenceProvided: [],
        evidenceRequired: answers.evidenceGaps ? answers.evidenceGaps.split(',').map(e => e.trim()) : [],
        responseDeadline: answers.rfeDate || '',
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="mb-8">
            <div className="flex items-center justify-between mb-2">
              <h1 className="text-xl font-bold" data-testid="heading-rfe-qa">RFE Q&A Defense</h1>
                    ...
                    <Card className={overallStrength >= 80 ? "border-green-500 dark:border-green-500" : overallStrength >= 60 ? "border-orange-500 dark:border-orange-500" : "border-destructive dark:border-destructive"}>
                      <CardContent className="pt-6">
                        <div className="text-center">
                          ...
                          <p className="text-xl font-bold" data-testid="text-overall-strength">{overallStrength}%</p>
                    ...
                    <Card className={completedQuestions === questions.length && questions.length > 0 ? "border-green-500 dark:border-green-500" : "border-orange-500 dark:border-orange-500"}>
                      <CardContent className="pt-6">
                        <div className="text-center">
                          ...
                          <p className="text-xl font-bold text-green-600 dark:text-green-400" data-testid="text-completed-questions">{completedQuestions}/{questions.length}</p>
                    ...
                    <Card className={urgentQuestions === 0 ? "border-green-500 dark:border-green-500" : "border-destructive dark:border-destructive"}>
                      <CardContent className="pt-6">
                        <div className="text-center">
                          ...
                          <p className="text-xl font-bold text-destructive" data-testid="text-urgent-questions">{urgentQuestions}</p>
