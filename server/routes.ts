...
import {
  ...
  startBusinessPlanGenerationForUser,
} from "./services/businessPlanGenerationService";
import {
...
Respond ONLY with valid JSON in this exact format:
{
  ...
  app.post("/api/generate/start", isAuthenticated, async (req, res) => {
    try {
      ...
      const user = req.user as any;
      const result = await startBusinessPlanGenerationForUser(planId, user.id);
      res.json(result);
    } catch (error) {
      if (error instanceof GenerationStartError) {
        return res.status(error.statusCode).json(error.payload);
      }
      console.error("Generation queue start error:", error);
