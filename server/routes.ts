...
import {
  ...
  blogGenerationQueue,
  seoAutomationPlans,
  TIER_CREDITS as SCHEMA_TIER_CREDITS,
  getTierCredits,
  eventLog,
  apiLatencyLog,
  creditTransactions,
...
import {
  ...
  getResetTokenExpiry,
  sendPlanCompletionEmail,
  sendReferralRewardEmail,
  sendPromoCodeRewardEmail,
  sendAdminVerificationSuccessEmail,
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  setImmediate(async () => {
    try {
      ...
      if (healed > 0) {
        console.log(
          `[Blog] Auto-healed ${healed} blog post image URLs on startup`,
        );
      }
    } catch (e) {
    ...
    async (req, res) => {
      try {
        ...
        if (plan.userId !== user.id)
          return res
            .status(403)
            .json({ error: "You can only edit your own business plans" });

        // null means revert to auto (hash-based)
        const styleValue =
          tocStyle === null || tocStyle === undefined ? null : Number(tocStyle);
        if (
          styleValue !== null &&
          (styleValue < 0 || styleValue > 9 || !Number.isInteger(styleValue))
        ) {
          return res.status(400).json({ error: "Style must be 0–9 or null" });
        }

        await db.execute(
          sql`UPDATE business_plans SET toc_style = ${styleValue} WHERE id = ${planId}`,
        );
  ...
  app.delete("/api/business-plans/:id", isAuthenticated, async (req, res) => {
    try {
      ...
      if (plan.userId !== user.id) {
        return res
          .status(403)
          .json({ error: "You can only delete your own business plans" });
      }
    ...
    async (req, res) => {
      try {
        ...
        if (plan.userId !== user.id) {
          return res
            .status(403)
            .json({ error: "You can only analyze your own business plans" });
        }
...
Respond ONLY with valid JSON in this exact format:
{
  ...
  "improvements": ["<improvement area 1>", "<improvement area 2>"]
        ...
        try {
          ...
          return res
            .status(500)
            .json({ error: "Failed to parse innovation analysis" });
        }
  ...
  app.post("/api/questionnaire/submit", isAuthenticated, async (req, res) => {
    try {
      console.log("Questionnaire submission received - Theme data:", {
        ...
        textElementsLength: req.body.textElements
    ...
    async (req, res) => {
      try {
        ...
        if (alreadyPurchased) {
          return res
            .status(400)
            .json({ error: "You already own this template" });
        }
        ...
        if (promoCode) {
          const promo = await storage.getPromoCodeByCode(
          ...
          if (now < promo.validFrom) {
            return res
              .status(400)
              .json({ error: "This promo code is not yet active" });
          }
          ...
          if (promo.maxUsesPerUser) {
            const userRedemptions = await storage.getUserPromoRedemptionCount(
              user.id,
              promo.id,
            );
          ...
          if (promo.discountType === "percentage") {
            discountApplied = Math.floor(
              priceInPence * (promo.discountValue / 100),
            );
          } else {
          ...
          line_items: [
            {
              price_data: {
                ...
                product_data: {
                  ...
                  description:
                    "Professional designer cover template for your business plan",
                },
  ...
  app.post("/api/premium-covers/verify", isAuthenticated, async (req, res) => {
    try {
      ...
      const purchase =
        await storage.getPremiumCoverPurchaseByStripeSession(sessionId);
      if (!purchase) {
  ...
  app.post(
    ...
    async (req, res) => {
      try {
        ...
        const userTierLevel =
          tierHierarchy[user.subscriptionTier || "free"] || 0;
        const planTierLevel = tierHierarchy[businessPlan.tier] || 0;

        if (
          user.subscriptionStatus === "active" &&
          userTierLevel >= planTierLevel &&
          userTierLevel > 0
        ) {
          console.log(
            `[CHECKOUT] User ${user.id} (${user.subscriptionTier}) already has active subscription - skipping payment for ${businessPlan.tier} plan`,
          );
          await storage.updateBusinessPlan(planId, { status: "paid" });
          return res.json({
            skipCheckout: true,
        ...
        if (promoCode) {
          const promoCodeRecord = await storage.getPromoCodeByCode(
            promoCode.toUpperCase(),
          );
          ...
          if (!promoCodeRecord) {
            return res
              .status(400)
              .json({ error: "Invalid promo code", promoError: true });
          }
          ...
          if (promoCodeRecord.validUntil && now > promoCodeRecord.validUntil) {
            return res
              .status(400)
              .json({ error: "This promo code has expired", promoError: true });
          }
        ...
        if (finalAmount === 0 && validPromoCode) {
          console.log(
            `[CHECKOUT] 100% discount applied with promo ${validPromoCode.code} - bypassing Stripe payment`,
          );
          ...
          console.log(
            `[CHECKOUT] User ${user.id} upgraded to ${businessPlan.tier} with ${creditsToAdd} credits via 100% promo code ${validPromoCode.code}`,
          );
  ...
  app.post(
    ...
    async (req, res) => {
      try {
        ...
        if (finalAmount === 0 && validPromoCode) {
          console.log(
            `[DIRECT SUBSCRIBE] 100% discount applied with promo ${validPromoCode.code} - bypassing Stripe payment`,
          );
          ...
          console.log(
            `[DIRECT SUBSCRIBE] User ${user.id} upgraded to ${tier} with ${creditsToAdd} credits via 100% promo code ${validPromoCode.code}`,
          );
        ...
        console.log(
          `[DIRECT SUBSCRIBE] User ${user.id} starting direct subscription to ${tier} tier (no plan created yet)`,
        );
        res.json({
          sessionId: session.id,
          url: session.url,
  ...
  app.post("/api/payment/verify", isAuthenticated, async (req, res) => {
    try {
      ...
      if (!sessionId || !planId) {
        return res
          .status(400)
          .json({ error: "Session ID and Plan ID required" });
      }
      ...
      if (businessPlan.stripeSessionId !== sessionId) {
        return res
          .status(403)
          .json({ error: "Session mismatch - security violation" });
      }
  ...
  app.post("/api/credits/consume", isAuthenticated, async (req, res) => {
    try {
      ...
      const totalCredits =
        (freshUser.planCredits || 0) + (freshUser.bonusCredits || 0);
  ...
  app.post("/api/credits/purchase-addon", isAuthenticated, async (req, res) => {
    try {
      ...
      res
        .status(500)
        .json({ error: error?.message || "Failed to create addon checkout" });
    }
  });
  ...
  app.post("/api/credits/verify-addon", isAuthenticated, async (req, res) => {
    try {
      ...
      if (addonType === "ultimate_assurance") {
        ...
        // Log the transaction
        const newBalance =
          (user.planCredits || 0) + (user.bonusCredits || 0) + creditsToAdd;
        await db.execute(sql`
          INSERT INTO credit_transactions (id, user_id, type, credits_change, credits_type, balance_after, reference_type, description)
  ...
  app.post(
    "/api/credits/grant-tier-credits",
    isAuthenticated,
    async (req, res) => {
      try {
        const { tier } = req.body;
    ...
    try {
      ...
      if (!isFreePlanGeneration) {
        ...
        if (totalCredits < 1) {
          return res.status(403).json({
            error:
              "Insufficient credits. Please purchase additional credits or upgrade your plan.",
            creditsRequired: 1,
        ...
        console.log(
          `[CREDITS] User ${user.id} consumed 1 ${creditsDeducted} credit for plan ${planId}. Balance: ${newPlanCredits + newBonusCredits}`,
        );
      } else {
        console.log(
          `[CREDITS] User ${user.id} generating FREE plan ${planId} - no credits required`,
        );
      }
  ...
  async function generateBusinessPlan(planId: string) {
    ...
    const { getSectionsForTier, getSectionSystemPrompt } = await import(
      "./aiPrompts"
    );
...
- Contact Points Plan: ${plan.contactPointsStrategy}`;
    ...
    console.log(
      `Starting multi-pass generation for ${sections.length} sections (${plan.tier} tier)`,
    );
    ...
    const getStageDescription = (
      ...
      if (progress === 0)
        return "Starting generation - analyzing your business model...";
      if (progress < 0.3)
        return `Analyzing - Section ${sectionIndex}/${total}: ${sections[sectionIndex - 1]?.title || ""}`;
      if (progress < 0.7)
        return `Building business plan - Section ${sectionIndex}/${total}: ${sections[sectionIndex - 1]?.title || ""}`;
      if (progress < 0.9)
        return `Proofreading - Section ${sectionIndex}/${total}: ${sections[sectionIndex - 1]?.title || ""}`;
    ...
    for (let i = 0; i < sections.length; i++) {
      ...
      console.log(
        `Generating section ${i + 1}/${sections.length}: ${section.title}`,
      );
      ...
      try {
        ...
        for (let j = 0; j < Math.min(3, lines.length); j++) {
          ...
          const isNumberedTitle =
            /^\d+\.\s+[A-Z][A-Z\s&]+$/.test(line) && isTitleMatch;
        ...
        console.log(
          `✓ Section ${i + 1} complete (${sectionContent.length} chars)`,
        );
      } catch (error: any) {
...
${generatedSections.join("\n\n---\n\n")}`;
  ...
  async function generatePDFWithPDFKit(
    ...
    doc.on("end", () => {
      ...
      const sanitizedName = businessPlan.businessName
    ...
    doc
      .fontSize(12)
      .text(
        `${tierName} Plan | Generated ${new Date().toLocaleDateString("en-GB")}`,
        50,
        145,
        { align: "center" },
      );
  ...
  app.get("/api/download/pdf/:planId", isAuthenticated, async (req, res) => {
    try {
      ...
      if (!businessPlan.generatedContent) {
        return res
          .status(500)
          .json({ error: "Business plan content is missing" });
      }
  ...
  app.get("/api/view/html/:planId", isAuthenticated, async (req, res) => {
    try {
      ...
      try {
        ...
        savedCoverDesign =
          await storage.getLatestCoverDesign(coverDesignUserId);
      } catch (coverError: any) {
        // Gracefully handle if cover_designs table doesn't exist in production
        console.log(
          "Cover design fetch failed (may not exist in production):",
          coverError?.message,
        );
      }
      ...
      const planWithTheme = {
        ...
        themePrimaryColor:
          savedCoverDesign?.primaryColor || businessPlan.themePrimaryColor,
        themeSecondaryColor:
          savedCoverDesign?.secondaryColor || businessPlan.themeSecondaryColor,
        themeFont: savedCoverDesign?.font || businessPlan.themeFont,
        backgroundImage:
          savedCoverDesign?.backgroundImage ||
          businessPlan.backgroundImage ||
          null,
        useFullCoverImage:
          savedCoverDesign?.useFullCoverImage ||
          businessPlan.useFullCoverImage ||
          false,
        textElements: (() => {
          // Prioritize savedCoverDesign's textElements
          if (savedCoverDesign?.textElements)
            return savedCoverDesign.textElements;
      ...
      console.log("[HTML View] Theme data for plan:", {
        ...
        finalTheme: {
          ...
          textElementsCount: Array.isArray(planWithTheme.textElements)
            ? planWithTheme.textElements.length
            : 0,
        },
      });
...
<body>
    ...
    try {
      ...
      children.push(
        ...
        new Paragraph({
          children: [
            new TextRun({
              text: businessPlan.businessName,
              bold: true,
              size: 72,
              color: "005EB8",
            }),
        ...
        new Paragraph({
          children: [
            new TextRun({
              text: "UK Innovation Visa Business Plan",
              size: 36,
              color: "666666",
            }),
          ],
        ...
        new Paragraph({
          children: [
            new TextRun({
              text: `Industry: ${businessPlan.industry}`,
              size: 24,
            }),
          ],
          alignment: AlignmentType.CENTER,
        ...
        new Paragraph({
          children: [
            new TextRun({
              text: `Tier: ${businessPlan.tier.charAt(0).toUpperCase() + businessPlan.tier.slice(1)}`,
              size: 24,
              bold: true,
            }),
          ],
      ...
      res.setHeader(
        "Content-Disposition",
        `attachment; filename="${encodeURIComponent(businessPlan.businessName)}-business-plan.docx"`,
      );
  ...
  app.get(
    ...
    async (req, res) => {
      try {
        ...
        const { getAllEndorsers, scoreBusinessPlanForEndorser } = await import(
          "./calculators/endorserSimulator"
        );

        const allEndorsers = getAllEndorsers();
        // Return scored endorsers directly as 'endorsers' array for UI compatibility
        const endorsers = allEndorsers.map((e: any) =>
          scoreBusinessPlanForEndorser(plan, e.id),
        );
  ...
  app.get("/api/team/model/:planId", isAuthenticated, async (req, res) => {
    try {
      ...
      const { generateTeamPlan, assessTeamSkills } = await import(
        "./calculators/teamModeller"
      );
      ...
      const teamPlan = {
        recommendedTeamSize:
          rawTeamPlan.totalJobsCommitted || rawTeamPlan.stages?.length || 5,
        keyRoles:
          rawTeamPlan.stages?.map(
            (s: any) => `${s.title} - ${s.role} (${s.quarter})`,
          ) || [],
  ...
  app.get(
    ...
    async (req, res) => {
      try {
        ...
        const { forecastTraction } = await import(
          "./calculators/tractionForecaster"
        );
  ...
  app.get("/api/rules/check/:planId", isAuthenticated, async (req, res) => {
    try {
      ...
      const { getRuleEngineStatus } = await import(
        "./calculators/ruleChangeEngine"
      );
      ...
      const transformedRules = status.applicableRules.map(
        (rule: any, idx: number) => {
          ...
          if (rule.category === "Job Creation") {
            score =
              businessProfile.jobCreation >= 5
                ? 90
                : Math.min(85, businessProfile.jobCreation * 15);
            ...
            ruleStatus = "pass";
            feedback =
              "No minimum capital requirement - your bootstrapped approach is acceptable.";
          } else if (rule.category === "Endorsement") {
            score = 85;
            ruleStatus = "pass";
            feedback =
              "Platform provides comprehensive documentation for endorsing body requirements.";
          } else if (rule.category === "Compliance") {
            score = 88;
            ruleStatus = "pass";
            feedback =
              "GDPR compliance integrated with data protection by design principles.";
          } else if (rule.category === "Extension Requirements") {
            score = 82;
            ruleStatus = "pass";
            feedback =
              "3-year milestones documented for ILR eligibility pathway.";
          }
  ...
  app.post("/api/ai/confirm-action", isAuthenticated, async (req, res) => {
    try {
      ...
      res
        .status(500)
        .json({ success: false, message: "Failed to confirm action" });
    }
  });
  ...
  app.post("/api/ai/cancel-action", isAuthenticated, async (req, res) => {
    try {
      ...
      const success = await cancelPendingAction(
        confirmationId,
        user.id,
        context,
      );
      ...
      res.json({
        success,
        message: success ? "Action cancelled" : "Failed to cancel action",
      });
    } catch (error) {
      console.error("AI cancel action error:", error);
      res
        .status(500)
        .json({ success: false, message: "Failed to cancel action" });
    }
  });
  ...
  app.post("/api/eligibility/assess", async (req, res) => {
    try {
      ...
      if (!businessConcept || !industrySlug) {
        return res
          .status(400)
          .json({ error: "Business concept and industry are required" });
      }
      ...
      const response = {
        ...
        aiAnalysis:
          result.aiAnalysis.endorserFit?.join(" ") ||
  ...
  app.get(
    "/api/eligibility/user/history",
    isAuthenticated,
    async (req, res) => {
      try {
        const user = req.user as any;
        const assessments = await storage.getUserEligibilityAssessments(
          user.id,
        );
...
Keep it concise but impactful. Return only the enhanced text.`;
  ...
  app.post(
    "/api/ai/questionnaire-enhance",
    isAuthenticated,
    async (req, res) => {
      try {
        const { mode, fieldLabel, fieldName, currentText, wordCount, context } =
          req.body;
  ...
  app.post(
    ...
    try {
      ...
      const toolContext: Record<
        string,
        { focus: string; keyMetrics: string[]; visaCriteria: string }
      > = {
        "market-analysis": {
          ...
          visaCriteria:
            "Scalability - demonstrates large addressable market for growth",
        },
        "financial-projections": {
          ...
          visaCriteria:
            "Viability - shows realistic financial planning and sustainability",
        },
        "innovation-score": {
          ...
          visaCriteria:
            "Innovation - genuine new or significantly improved offering",
        },
        "growth-strategy": {
          ...
          visaCriteria:
            "Scalability - credible plan for national/international growth",
        },
        "hiring-plan": {
          ...
          visaCriteria:
            "All criteria - job creation is a key endorsement factor",
        },
        "competitor-bench": {
          ...
          visaCriteria:
            "Innovation - shows understanding of competitive landscape",
        },
      ...
      const prompt = `You are ${
        agentPersonality === "Creative, enthusiastic, forward-thinking"
          ? "Nova, the Innovation Specialist"
          : agentPersonality === "Analytical, precise, business-focused"
            ? "Sterling, the Financial Analyst"
            : agentPersonality === "Strategic, ambitious, growth-oriented"
              ? "Atlas, the Growth Strategist"
              : "Sage, the Compliance Expert"
      } - an AI agent for UK Innovator Founder Visa applications.
...
- "The 18-month breakeven timeline is realistic. However, endorsers will want to see the assumptions behind your £45K/month revenue target."`;
  ...
  app.post(
    ...
    try {
      ...
      if (!toolId || !payload) {
        return res
          .status(400)
          .json({ error: "toolId and payload are required" });
      }
  ...
  app.post("/api/referrals", async (req, res) => {
    try {
      ...
      if (!toolId || !channel) {
        return res
          .status(400)
          .json({ error: "toolId and channel are required" });
      }
  ...
  const toolAnalyticsSchema = z.object({
    ...
    action: z
      .enum(["access", "save", "export", "share", "upload", "download"])
      .default("access"),
  });
  ...
  app.get("/api/admin/analytics/overview", requireAdmin, async (req, res) => {
    try {
      ...
      `);
      const toolStats = (toolUsageQuery.rows[0] as any) || {
        unique_tools: 0,
        total_uses: 0,
      };
      ...
      `);
      const avgToolsPerUser =
        parseFloat((avgToolsQuery.rows[0] as any)?.avg_tools) || 0;
      ...
      const dailyActiveUsers =
        parseInt((dailyActiveQuery.rows[0] as any)?.count) || activeUsers;
      ...
      allUsers.forEach((user) => {
        const tier = (user.subscriptionTier ||
          "free") as keyof typeof tierCounts;
      ...
      for (let i = 29; i >= 0; i--) {
        ...
        const count = allUsers.filter(
          (u) => new Date(u.createdAt).toISOString().split("T")[0] === dateStr,
        ).length;
      ...
      const timeSeriesData = activityData.map((d) => {
        ...
        return {
          ...
          plans: allPlans.filter(
            (p) => new Date(p.createdAt).toISOString().split("T")[0] <= d.date,
          ).length,
          revenue: 0,
        };
      });
      ...
      const overallScore = Math.round(
        Math.min(
          kpiTargets.userAcquisition.value / kpiTargets.userAcquisition.target,
          1,
        ) *
          20 +
          Math.min(
            kpiTargets.planCompletion.value / kpiTargets.planCompletion.target,
            1,
          ) *
            20 +
          Math.min(kpiTargets.revenue.value / kpiTargets.revenue.target, 1) *
            20 +
          Math.min(
            kpiTargets.dailyActive.value / kpiTargets.dailyActive.target,
            1,
          ) *
            20 +
          Math.min(
            kpiTargets.toolAdoption.value / kpiTargets.toolAdoption.target,
            1,
          ) *
            20,
      );
      ...
      res.json({
        kpiMetrics: [
          ...
          },
          {
            ...
            trend: {
              value: Math.round(
                (dailyActiveUsers / Math.max(totalUsers, 1)) * 100,
              ),
              direction: "up" as const,
              period: "24h",
            },
        ...
        timeSeriesData,
        subscriptionDistribution: Object.entries(tierCounts).map(
          ([tier, count]) => ({
            tier,
            count,
  ...
  app.get("/api/admin/analytics/users", requireAdmin, async (req, res) => {
    try {
      ...
      const usersWithPlans = allUsers.filter(
      ...
      const userJourneyFunnel = [
        ...
        {
          stage: "First Login",
          count: Math.max(activeUsers, Math.floor(verifiedUsers * 0.85)),
        },
        {
          stage: "Used Tool",
          count: Math.max(
            Math.floor(activeUsers * 0.7),
            Math.floor(verifiedUsers * 0.6),
          ),
        },
      ...
      for (let i = 5; i >= 0; i--) {
        ...
        const usersUntilMonth = allUsers.filter(
          (u) =>
        ...
        usersByTier.push({
          date: monthStr,
          free: usersUntilMonth.filter(
            (u) => !u.subscriptionTier || u.subscriptionTier === "free",
          ).length,
          basic: usersUntilMonth.filter((u) => u.subscriptionTier === "basic")
            .length,
          premium: usersUntilMonth.filter(
            (u) => u.subscriptionTier === "premium",
          ).length,
          enterprise: usersUntilMonth.filter(
            (u) => u.subscriptionTier === "enterprise",
          ).length,
          ultimate: usersUntilMonth.filter(
            (u) => u.subscriptionTier === "ultimate",
          ).length,
        });
      }
      ...
      for (let i = 4; i >= 0; i--) {
        const weekStart = new Date(
          now.getTime() - (i + 4) * 7 * 24 * 60 * 60 * 1000,
        );
        ...
        cohortAnalysis.push({
          ...
          week2: Math.floor(cohortSize * (0.7 - i * 0.05)),
  ...
  app.get("/api/admin/analytics/plans", requireAdmin, async (req, res) => {
    try {
      ...
      // Completion rate
      const completedCount = allPlans.filter(
        (p) => p.status === "completed",
      ).length;
      const completionRate =
        allPlans.length > 0
          ? Math.round((completedCount / allPlans.length) * 100)
          : 0;
  ...
  app.get("/api/admin/analytics/tools", requireAdmin, async (req, res) => {
    try {
      ...
      toolUsageStats.forEach((stat) => {
        byActionType[stat.action] =
          (byActionType[stat.action] || 0) + stat.count;
      });
      ...
      toolUsageStats
        .filter(
          (stat) => stat.timestamp && new Date(stat.timestamp) >= sevenDaysAgo,
        )
        .forEach((stat) => {
          if (stat.timestamp) {
            const dateKey = new Date(stat.timestamp)
              .toISOString()
              .split("T")[0];
    ...
    try {
      ...
      while (hasMore) {
        ...
        if (chargesResponse.data.length > 0) {
          startingAfter =
            chargesResponse.data[chargesResponse.data.length - 1].id;
        } else {
          hasMore = false;
        }
      }
      ...
      const validCharges = allCharges.filter(
        (c) => new Date(c.created * 1000) >= BUSINESS_LAUNCH_DATE,
      );
      ...
      const year1Charges = validCharges.filter((c) => {
        ...
        return (
          chargeDate >= BUSINESS_LAUNCH_DATE &&
          chargeDate <= year1Cutoff &&
          c.amount_captured > 0
        );
      });
      const revenueYear1ToDate = year1Charges.reduce(
        (sum, c) => sum + c.amount_captured / 100,
        0,
      );

      // Calculate revenue for different periods (using only post-launch charges)
      const todayCharges = validCharges.filter(
        (c) => new Date(c.created * 1000) >= startOfToday,
      );
      const weekCharges = validCharges.filter(
        (c) => new Date(c.created * 1000) >= startOfWeek,
      );
      const monthCharges = validCharges.filter(
        (c) => new Date(c.created * 1000) >= startOfMonth,
      );
      ...
      const paidTodayCharges = todayCharges.filter(
        (c) => c.amount_captured > 0,
      );
      const paidWeekCharges = weekCharges.filter((c) => c.amount_captured > 0);
      const paidMonthCharges = monthCharges.filter(
        (c) => c.amount_captured > 0,
      );
      const paidLastMonthCharges = lastMonthCharges.filter(
        (c) => c.amount_captured > 0,
      );
      const paidValidCharges = validCharges.filter(
        (c) => c.amount_captured > 0,
      );

      const revenueToday = paidTodayCharges.reduce(
        (sum, c) => sum + c.amount_captured / 100,
        0,
      );
      const revenueThisWeek = paidWeekCharges.reduce(
        (sum, c) => sum + c.amount_captured / 100,
        0,
      );
      const revenueThisMonth = paidMonthCharges.reduce(
        (sum, c) => sum + c.amount_captured / 100,
        0,
      );
      const revenueLastMonth = paidLastMonthCharges.reduce(
        (sum, c) => sum + c.amount_captured / 100,
        0,
      );
      // Use validCharges (post-launch only) for all-time revenue - only actual payments
      const revenueAllTime = paidValidCharges.reduce(
        (sum, c) => sum + c.amount_captured / 100,
        0,
      );
      ...
      const totalDiscounts = validCharges.reduce((sum, c) => {
        ...
        const discount = c.metadata?.discount_amount
          ? parseFloat(c.metadata.discount_amount)
          : 0;
        return sum + discount;
      }, 0);
      ...
      while (hasMore) {
        ...
        if (cancelledResponse.data.length > 0) {
          startingAfter =
            cancelledResponse.data[cancelledResponse.data.length - 1].id;
        } else {
          hasMore = false;
        }
      }
      ...
      const tierDistribution = {
        free: allUsers.filter(
          (u) => !u.subscriptionTier || u.subscriptionTier === "free",
        ).length,
        basic: allUsers.filter((u) => u.subscriptionTier === "basic").length,
        premium: allUsers.filter((u) => u.subscriptionTier === "premium")
          .length,
        enterprise: allUsers.filter((u) => u.subscriptionTier === "enterprise")
          .length,
        ultimate: allUsers.filter((u) => u.subscriptionTier === "ultimate")
          .length,
      };
      ...
      const revenueByTier = {
        basic: paidValidCharges
          .filter((c) => resolveTier(c) === "basic")
          .reduce((s, c) => s + c.amount_captured / 100, 0),
        premium: paidValidCharges
          .filter((c) => resolveTier(c) === "premium")
          .reduce((s, c) => s + c.amount_captured / 100, 0),
        enterprise: paidValidCharges
          .filter((c) => resolveTier(c) === "enterprise")
          .reduce((s, c) => s + c.amount_captured / 100, 0),
        ultimate: paidValidCharges
          .filter((c) => resolveTier(c) === "ultimate")
          .reduce((s, c) => s + c.amount_captured / 100, 0),
      };

      // Calculate Total Customers - use paid users from database as primary source
      // Fall back to Stripe customer count if available
      const stripeCustomerCount =
        validCharges.length > 0
          ? new Set(validCharges.map((c) => c.customer).filter(Boolean)).size
          : 0;
      const totalCustomers =
        paidUsers.length > 0 ? paidUsers.length : stripeCustomerCount;
      ...
      const avgOrderValue =
        validCharges.length > 0 ? revenueAllTime / validCharges.length : 0;
      ...
      for (let i = 11; i >= 0; i--) {
        ...
        const paidMonthChargesData = monthChargesData.filter(
          (c) => c.amount_captured > 0,
        );
        monthlyTrend.push({
          month: monthName,
          revenue: paidMonthChargesData.reduce(
            (s, c) => s + c.amount_captured / 100,
            0,
          ),
      ...
      const cancelledThisMonth = cancelledSubscriptions.filter((s) => {
        const cancelDate = s.canceled_at
          ? new Date(s.canceled_at * 1000)
          : null;
      ...
      const churnRate =
        effectiveActiveSubscriptions > 0
          ? Math.round(
              (cancelledThisMonth /
                (effectiveActiveSubscriptions + cancelledThisMonth)) *
                100 *
                10,
            ) / 10
          : 0;
      ...
      for (const redemption of allRedemptions) {
        const promoCode = allPromoCodesData.find(
          (p) => p.id === redemption.promoCodeId,
        );
        if (
          promoCode &&
        ...
        const transactions = charges.data
          ...
          .map((charge) => {
            const user = charge.customer
              ? usersByStripeId.get(charge.customer as string)
              : null;
            ...
            return {
              ...
              user: user
                ? `${user.firstName || ""} ${user.lastName || ""}`.trim() ||
                  user.email?.split("@")[0] ||
                  "Customer"
                : charge.billing_details?.name || "Customer",
              email:
                user?.email ||
      ...
      try {
        ...
        const formattedSubscriptions = subscriptions.data.map((sub) => {
          ...
          const since = sub.start_date
            ? new Date(sub.start_date * 1000).toLocaleDateString("en-GB", {
                month: "short",
                year: "numeric",
              })
            : "N/A";
          ...
          if (sub.cancel_at_period_end) status = "at_risk";
            ...
            sub.current_period_end * 1000 - Date.now() < 7 * 24 * 60 * 60 * 1000
          ) {
            status = "renewing";
          }
  ...
  app.get("/api/admin/analytics/emails", requireAdmin, async (req, res) => {
    try {
      ...
      // Calculate metrics
      const totalSent30Days = allEmails.filter(
        (e) => new Date(e.sentAt) >= thirtyDaysAgo,
      ).length;
      const sentToday = allEmails.filter(
        (e) => new Date(e.sentAt) >= today,
      ).length;
      const successfulEmails = allEmails.filter(
        (e) => e.status === "sent" || e.status === "delivered",
      );
      const deliveryRate =
        totalSent30Days > 0
          ? (
              (successfulEmails.filter(
                (e) => new Date(e.sentAt) >= thirtyDaysAgo,
              ).length /
                totalSent30Days) *
              100
      ...
      // Group by email type
      const typeBreakdown = allEmails.reduce(
        (acc, email) => {
      ...
      const typeDistribution = Object.entries(typeBreakdown).map(
        ([type, count]) => ({
          type: formatEmailType(type),
          count,
          percent:
            totalSent30Days > 0
              ? ((count / allEmails.length) * 100).toFixed(1)
              : "0",
        }),
      );
  ...
  app.post(
    "/api/admin/emails/send-bulk-welcome",
    requireAdmin,
    async (req, res) => {
      try {
        ...
        if (allUsers.length === 0) {
          return res
            .status(400)
            .json({ error: "No users found to send emails to" });
        }

        console.log(
          `[Bulk Email] Starting to send welcome emails to ${allUsers.length} users`,
        );
  ...
  app.post("/api/admin/emails/send-test", requireAdmin, async (req, res) => {
    try {
      ...
      if (!emails || !Array.isArray(emails) || emails.length === 0) {
        return res
          .status(400)
          .json({ error: "Please provide an array of email addresses" });
      }
      ...
      res
        .status(500)
        .json({ error: "Failed to send test emails", message: error.message });
    }
  });
  ...
  app.get(
    ...
    async (req, res) => {
      try {
        ...
        const recent30Days = allNotifications.filter(
          (n) => n.createdAt && new Date(n.createdAt) >= thirtyDaysAgo,
        );

        // Calculate stats
        const totalSent30Days = recent30Days.filter(
          (n) => n.status === "sent",
        ).length;
        const sentToday = allNotifications.filter(
          (n) =>
            n.createdAt &&
        ...
        const inAppCount = allNotifications.filter(
          (n) => n.status === "sent",
        ).length;
        const scheduledCount = allScheduled.filter(
          (n) => n.status === "sent",
        ).length;

        // Calculate read rate
        const totalRecipients = allNotifications.reduce(
          (sum, n) => sum + (n.recipientCount || 0),
          0,
        );
        const totalReads = allNotifications.reduce(
          (sum, n) => sum + (n.readCount || 0),
          0,
        );
        const readRate =
          totalRecipients > 0
            ? ((totalReads / totalRecipients) * 100).toFixed(1)
            : "0";
        ...
        const typeDistribution = Object.entries(typeBreakdown)
          .map(([type, count]) => ({
            type:
              type.charAt(0).toUpperCase() + type.slice(1).replace(/_/g, " "),
            count,
        ...
        res
          .status(500)
          .json({ error: "Failed to fetch notification analytics" });
      }
    },
  );
    ...
    try {
      ...
      if (tierFilters) {
        ...
        allUsers = allUsers.filter((u) =>
          tiers.includes(u.subscriptionTier || "free"),
        );
      }
      ...
      if (verified !== undefined) {
        ...
        allUsers = allUsers.filter(
          (u) => u.isEmailVerified === isVerifiedParam,
        );
      }
  ...
  app.get(
    "/api/admin/users/:userId/export-history",
    requireAdmin,
    async (req, res) => {
      try {
        const { userId } = req.params;
  ...
  app.post("/api/admin/reset-all-credits", requireAdmin, async (req, res) => {
    try {
      ...
      console.log(
        `[ADMIN] Reset credits for ${updatedCount} users based on tier`,
      );
      res.json({
        success: true,
    ...
    try {
      ...
      const statusFilter = statusFilters
        ? Array.isArray(statusFilters)
          ...
          : statusFilters
        : status;
      const tierFilter = tierFilters
        ? Array.isArray(tierFilters)
          ? tierFilters[0]
          : tierFilters
        : tier;
  ...
  app.patch("/api/admin/plans/:planId", requireAdmin, async (req, res) => {
    try {
      ...
      const updatedPlan = await storage.updateBusinessPlan(
        planId,
        filteredUpdates,
      );
  ...
  app.get(
    "/api/admin/plans/:planId/download",
    requireAdmin,
    async (req, res) => {
      try {
        const { planId } = req.params;
        ...
        const clientName = owner
          ? `${owner.firstName || ""}_${owner.lastName || ""}`.replace(
              /[^a-zA-Z0-9_]/g,
              "",
            )
          : "";
        ...
        doc
          .fontSize(28)
          .fillColor("#ffffff")
          .text("Business Plan Submission", 50, 35);
        ...
        doc
          .fontSize(18)
          .fillColor("#11b6e9")
          .text("Business Overview", 70, boxY);
        doc.fontSize(12).fillColor("#333333");
        doc.text(`Business Name: ${plan.businessName || "N/A"}`, 70, boxY + 30);
        doc.text(`Industry: ${plan.industry || "N/A"}`, 70, boxY + 50);
        doc.text(
          `Selected Tier: ${(plan.tier || "N/A").charAt(0).toUpperCase() + (plan.tier || "").slice(1)}`,
          70,
          boxY + 70,
        );
        doc.text(
          `Status: ${(plan.status || "N/A").charAt(0).toUpperCase() + (plan.status || "").slice(1)}`,
          70,
          boxY + 90,
        );
        doc.text(
          `Submitted: ${plan.createdAt ? new Date(plan.createdAt).toLocaleDateString("en-GB") : "N/A"}`,
          350,
          boxY + 30,
        );
        doc.text(
          `Innovation Stage: ${(plan.innovationStage || "N/A").charAt(0).toUpperCase() + (plan.innovationStage || "").slice(1)}`,
          350,
          boxY + 50,
        );
        doc.text(
          `Funding Available: £${plan.funding?.toLocaleString() || "N/A"}`,
          350,
          boxY + 70,
        );
        doc.text(
          `Job Creation Target: ${plan.jobCreation || "N/A"} jobs`,
          350,
          boxY + 90,
        );
        ...
        if (owner) {
          ...
          doc.text(
            `Account Created: ${owner.createdAt ? new Date(owner.createdAt).toLocaleDateString("en-GB") : "N/A"}`,
          );
          doc.moveDown(1.5);
        }
        ...
        doc
          .fontSize(20)
          .fillColor("#ffa536")
          .text("Business Details", { underline: false });
        doc.moveDown(1);
        ...
        if (
          ...
          doc.moveDown(0.5);
          doc.fontSize(11).fillColor("#333333");
          if (plan.customerAcquisitionCost)
            doc.text(
              `Customer Acquisition Cost (CAC): £${plan.customerAcquisitionCost}`,
            );
          if (plan.lifetimeValue)
            doc.text(`Lifetime Value (LTV): £${plan.lifetimeValue}`);
          if (plan.paybackPeriod)
            doc.text(`Payback Period: ${plan.paybackPeriod} months`);
          doc.moveDown(1.5);
        }
        ...
        doc
          .fontSize(20)
          .fillColor("#ffa536")
          .text("Visa Compliance & Endorser Strategy");
        doc.moveDown(1);
        ...
        if (plan.complianceBudget) {
          doc
            .fontSize(14)
            .fillColor("#ffa536")
            .text("Compliance Budget", { underline: true });
          doc.moveDown(0.5);
          doc
            .fontSize(11)
            .fillColor("#333333")
            .text(`£${plan.complianceBudget?.toLocaleString() || "N/A"}`);
          doc.moveDown(1.5);
        }
        ...
        doc
          .fontSize(10)
          .fillColor("#999999")
          .text("--- End of Business Plan Submission ---", { align: "center" });
        doc.moveDown(0.5);
        doc
          .fontSize(9)
          .text(
  ...
  app.get("/api/admin/audit-log", requireAdmin, async (req, res) => {
    try {
      ...
      const allLogs = [...formattedLogs, ...systemEvents].sort(
        (a, b) =>
          new Date(b.timestamp).getTime() - new Date(a.timestamp).getTime(),
      );
  ...
  app.post("/api/security/log", async (req, res) => {
    try {
      ...
      if (!eventType || !description) {
        return res
          .status(400)
          .json({ error: "eventType and description are required" });
      }
  ...
  app.delete(
    ...
    async (req, res) => {
      try {
        ...
        await db.delete(securityEvents).where(eq(securityEvents.id, eventId));
  ...
  app.delete(
    "/api/admin/security-events/resolved/all",
    requireAdmin,
    async (req, res) => {
      try {
        await db
        ...
        res.json({
          success: true,
          message: "All resolved security events cleared",
        });
      } catch (error) {
        console.error("Resolved security events clear failed:", error);
        res
          .status(500)
          .json({ error: "Failed to clear resolved security events" });
      }
    },
  );
  ...
  app.post("/api/admin/system/cache-clear", requireAdmin, async (req, res) => {
    try {
      ...
      res.json({
        success: true,
        message:
          "Cache clear signal sent. Client-side caches will be invalidated.",
        timestamp: new Date().toISOString(),
      });
    } catch (error) {
  ...
  app.post("/api/admin/system/export", requireAdmin, async (req, res) => {
    try {
      ...
      if (type === "users") {
        ...
        // CSV header
        csvData =
          "ID,Email,First Name,Last Name,Subscription Tier,Subscription Status,Email Verified,Admin,Created At\n";
        ...
        // CSV header
        csvData =
          "ID,Business Name,Industry,Tier,Status,User ID,Is Demo,Created At\n";
        ...
        csvData =
          "ID,Code,User ID,Reward Type,Reward Value,Total Clicks,Total Signups,Total Earnings,Status,Created At\n";
        ...
        csvData =
          "ID,Code,Discount Type,Discount Value,Max Uses,Current Uses,Expires At,Status,Created At\n";
        ...
        csvData =
          "ID,User Email,Amount,Currency,Type,Status,Payment Method,Stripe ID,Created At\n";
  ...
  app.get("/api/referrals/dashboard", isAuthenticated, async (req, res) => {
    try {
      ...
      const stats = {
        ...
        qualified: events.filter(
          (e) => e.status === "qualified" || e.status === "rewarded",
        ).length,
        pendingEarnings:
          rewards
  ...
  app.get("/api/referrals/validate/:code", async (req, res) => {
    try {
      ...
      if (
        ...
        referralCode.totalReferrals >= referralCode.maxUses
      ) {
        return res.json({
          valid: false,
    ...
    try {
      ...
      if (promoCode.grantsTier) {
        ...
        if (promoCode.grantsCredits && promoCode.grantsCredits > 0) {
          updateData.bonusCredits =
            currentBonusCredits + promoCode.grantsCredits;
        }
    ...
    async (req, res) => {
      try {
        ...
        if (promoCode.eligibleTiers && promoCode.eligibleTiers.length > 0) {
          if (
            !promoCode.eligibleTiers.includes(user.subscriptionTier || "free")
          ) {
            return res.json({
              eligible: false,
              message:
                "This promo code is not available for your subscription tier",
            });
          }
        }
        ...
        if (
          promoCode.minPurchaseAmount &&
          purchaseAmount < promoCode.minPurchaseAmount
        ) {
          return res.json({
            eligible: false,
        ...
        res
          .status(500)
          .json({ eligible: false, message: "Failed to check eligibility" });
      }
    },
  );
  ...
  app.post("/api/payouts", isAuthenticated, async (req, res) => {
    try {
      ...
      if (!paymentMethod || !paymentDetails) {
        return res
          .status(400)
          .json({ error: "Payment method and details required" });
      }

      // Check user's available balance from referral codes
      const userCodes = await storage.getUserReferralCodes(user.id);
      const totalEarnings = userCodes.reduce(
        (sum, c) => sum + c.totalEarnings,
        0,
      );
      const paidEarnings = userCodes.reduce(
        (sum, c) => sum + c.paidEarnings,
        0,
      );
      ...
      try {
        ...
        console.error(
          "Failed to send payout request notification:",
          emailError,
        );
      }
  ...
  app.get(
    "/api/admin/referrals/pending-rewards",
    requireAdmin,
    async (req, res) => {
      try {
  ...
  app.get("/api/admin/promos", requireAdmin, async (req, res) => {
    try {
      ...
      const codesWithAnalytics = codes.map((code) => {
        const codeRedemptions = redemptions.filter(
          (r) => r.promoCodeId === code.id,
        );
        const totalRevenue = codeRedemptions.reduce(
      ...
      const totalRevenueSaved = redemptions.reduce(
        (sum, r) => sum + (r.discountApplied || 0),
        0,
      );
      ...
      res.json({
        ...
        summary: {
          ...
          totalRevenueSaved,
          averageDiscount:
            totalRedemptions > 0 ? totalRevenueSaved / totalRedemptions : 0,
        },
      });
    } catch (error) {
    ...
    try {
      ...
      const parsedDiscountValue =
        typeof discountValue === "string"
          ...
          : discountValue;
      const parsedMinPurchase = minPurchaseAmount
        ? typeof minPurchaseAmount === "string"
          ? parseFloat(minPurchaseAmount)
          : minPurchaseAmount
        : null;
      const parsedTotalUses = totalUses
        ? typeof totalUses === "string"
          ? parseInt(totalUses, 10)
          : totalUses
        : null;
      const parsedMaxUsesPerUser = maxUsesPerUser
        ? typeof maxUsesPerUser === "string"
          ? parseInt(maxUsesPerUser, 10)
          : maxUsesPerUser
        : 1;
      const parsedGrantsCredits = grantsCredits
        ? typeof grantsCredits === "string"
          ? parseInt(grantsCredits, 10)
          : grantsCredits
        : null;
      ...
      if (existing) {
        return res
          .status(400)
          .json({ error: "A promo code with this code already exists" });
      }
      ...
      const promoCode = await storage.createPromoCode({
        ...
        minPurchaseAmount: parsedMinPurchase
          ? Math.round(parsedMinPurchase * 100)
          : null,
  ...
  const bulkPromoCodeSchema = z.object({
    ...
    discountType: z.enum(["percentage", "fixed"], {
      errorMap: () => ({ message: "Must be 'percentage' or 'fixed'" }),
    }),
        ...
        try {
          ...
          const promoCode = await storage.createPromoCode({
            ...
            minPurchaseAmount: validatedData.minPurchaseAmount
              ? Math.round(validatedData.minPurchaseAmount * 100)
              : null,
            ...
            validFrom: validatedData.validFrom
              ? new Date(validatedData.validFrom as string)
              : new Date(),
            validUntil: validatedData.validUntil
              ? new Date(validatedData.validUntil as string)
              : null,
            status: "active",
            createdBy: user.id,
          });
  ...
  app.post(
    "/api/admin/referrals/rewards/:id/approve",
    requireAdmin,
    async (req, res) => {
      try {
        const { id } = req.params;
  ...
  // Reject reward (admin)
  app.post(
    "/api/admin/referrals/rewards/:id/reject",
    requireAdmin,
    async (req, res) => {
      try {
        const { id } = req.params;
  ...
  app.get("/api/admin/lawyer-reviews/:id", requireAdmin, async (req, res) => {
    try {
      ...
      const [comments, history, businessPlan] = await Promise.all([
        ...
        review.businessPlanId
          ? storage.getBusinessPlan(review.businessPlanId)
          : null,
      ]);
  ...
  app.post(
    "/api/admin/lawyer-reviews/:id/assign",
    requireAdmin,
    async (req, res) => {
      try {
        const { id } = req.params;
  ...
  app.patch(
    "/api/admin/lawyer-reviews/:id/status",
    requireAdmin,
    async (req, res) => {
      try {
        const { id } = req.params;
  ...
  app.post(
    "/api/admin/lawyer-reviews/:id/complete",
    requireAdmin,
    async (req, res) => {
      try {
        const { id } = req.params;
  ...
  app.post(
    "/api/admin/lawyer-reviews/:id/comments",
    requireAdmin,
    async (req, res) => {
      try {
        const { id } = req.params;
  ...
  // Update a comment
  app.patch(
    "/api/admin/lawyer-reviews/comments/:commentId",
    requireAdmin,
    async (req, res) => {
      try {
        const { commentId } = req.params;
        const comment = await storage.updateLawyerReviewComment(
          commentId,
          req.body,
        );
        if (!comment) {
  ...
  app.get(
    "/api/admin/lawyer-reviews/analytics",
    requireAdmin,
    async (req, res) => {
      try {
  ...
  app.get("/api/lawyer-reviews/:id", isAuthenticated, async (req, res) => {
    try {
      ...
      if (review.userId !== user.id) {
        return res
          .status(403)
          .json({ error: "You do not have permission to view this review" });
      }
    ...
    async (req, res) => {
      try {
        ...
        if (review.userId !== user.id) {
          return res
            .status(403)
            .json({ error: "You do not have permission to rate this review" });
        }

        if (review.status !== "completed") {
          return res
            .status(400)
            .json({ error: "Can only rate completed reviews" });
        }
  ...
  app.post("/api/partner/contact-user", isAuthenticated, async (req, res) => {
    try {
      ...
      if (!userId || !subject || !message) {
        return res
          .status(400)
          .json({ error: "User ID, subject, and message are required" });
      }

      // Verify this user used partner's promo code
      const analytics = await storage.getPartnerAnalytics(user.id);
      const allUserIds = analytics.usersByPromoCode.flatMap((pc) =>
        pc.users.map((u) => u.userId),
      );
      ...
      console.log(
        `Partner ${user.email} sending email to ${targetUser.email}: ${subject}`,
      );
  ...
  app.post("/api/support/contact", isAuthenticated, async (req, res) => {
    try {
      ...
      if (!topic || !subject || !message) {
        return res
          .status(400)
          .json({ error: "Topic, subject, and message are required" });
      }
    ...
    try {
      ...
      const storageStats = {
        ...
        documents: documents.map((d) => ({
          ...
          name: d.name,
          category: d.category,
          storageType: d.fileUrl.startsWith("s3://")
            ? "s3"
  ...
  app.post(
    ...
    async (req, res) => {
      try {
        ...
        if (storageType === "local") {
          try {
            ...
            if (cloudResponse.ok) {
              ...
              storageType = "replit";
              console.log(
                "[Document Upload] Uploaded to Replit storage:",
                objectPath,
              );
              fs.unlinkSync(file.path);
            }
        ...
        if (storageType === "local") {
          console.log(
            "[Document Upload] Using local storage (not persistent on Railway)",
          );
        }
        ...
        console.log(
          `[Document Upload] Document saved: ${name} (storage: ${storageType}, url: ${fileUrl})`,
        );
  ...
  app.delete("/api/documents/:id", isAuthenticated, async (req, res) => {
    try {
      ...
      try {
        ...
        console.log(
          "[Document Delete] Storage deletion failed (continuing):",
          storageError,
        );
      }
  ...
  app.post("/api/documents/extract", isAuthenticated, async (req, res) => {
    try {
      ...
      console.log(
        "[Document Extract] Starting extraction for user:",
        user.id,
        "documents:",
        documentIds,
      );
      ...
      if (
        ...
        return res
          .status(400)
          .json({ error: "Please select at least one document" });
      }
      ...
      for (const docId of documentIds) {
        ...
        if (!doc || doc.userId !== user.id) {
          console.log(
            "[Document Extract] Document not found or unauthorized:",
            docId,
          );
      ...
      console.log(
        "[Document Extract] Found documents:",
        documents.map((d) => ({
          name: d.name,
          category: d.category,
          fileUrl: d.fileUrl,
      ...
      for (const doc of documents) {
        ...
        console.log(
          `[Document Extract] Processing ${doc.name} - URL: ${doc.fileUrl}`,
        );

        // Priority 1: Check if document is in S3 (s3:// prefix)
        if (doc.fileUrl.startsWith("s3://")) {
          console.log(
            `[Document Extract] Attempting S3 download for: ${doc.fileUrl}`,
          );
          try {
            const s3Key = s3Storage.getKeyFromUrl(doc.fileUrl);
            if (s3Key) {
              ...
              fileFound = true;
              console.log(
                "[Document Extract] File found in S3:",
                s3Key,
                "Size:",
                fileBuffer.length,
              );
        ...
        if (!fileFound && doc.fileUrl.startsWith("/objects/")) {
          try {
            ...
            fileFound = true;
            console.log(
              "[Document Extract] File found in Replit storage:",
              doc.fileUrl,
            );
          } catch (cloudError) {
            console.log(
              "[Document Extract] Replit storage read failed:",
              cloudError,
            );
          }
        }
          ...
          for (const filePath of possiblePaths) {
            if (fs.existsSync(filePath)) {
              try {
                ...
                fileFound = true;
                console.log(
                  "[Document Extract] File found locally at:",
                  filePath,
                );
                break;
              } catch (readError) {
                console.error(
                  "[Document Extract] Error reading file:",
                  filePath,
                  readError,
                );
              }
        ...
        if (fileFound && fileBuffer) {
          ...
          if (isPdf) {
            ...
            try {
              ...
              console.log(
                "[Document Extract] ScannedLikely:",
                pdfResult.isScannedLikely,
              );
              ...
              if (!pdfResult.isScannedLikely && pdfResult.text.length > 0) {
                ...
                console.log(
                  "[Document Extract] PDF text extracted successfully, chars:",
                  pdfResult.charCount,
                );
                console.log(
                  "[Document Extract] First 1000 chars:",
                  pdfResult.text.substring(0, 1000),
                );
              } else {
                // Scanned PDF - needs OCR or Vision API
                console.log(
                  "[Document Extract] PDF appears to be scanned (image-only)",
                );
                ...
                console.log(
                  "[Document Extract] Added scanned PDF marker to documentContents",
                );
              }
            } catch (pdfError: any) {
              console.error(
                "[Document Extract] PDF extraction error:",
            ...
            console.log(
              "[Document Extract] Image prepared for Vision API:",
              doc.name,
            );
          }
        }
      ...
      console.log(
        "[Document Extract] Files with content:",
...
Return ONLY valid JSON, no markdown or explanation.`;
      ...
      try {
        ...
        if (!process.env.OPENAI_API_KEY) {
          console.log(
            "[Document Extract] No OpenAI API key configured - using placeholders",
          );
        ...
        if (documentContents.length > 0 && process.env.OPENAI_API_KEY) {
          ...
          try {
            ...
            const imageDocuments = documentContents.filter(
              (d) => !d.isText && d.mimeType.startsWith("image/"),
            );
            const fileDocuments = documentContents.filter(
              (d) => d.mimeType === "openai/file",
            );

            console.log(
              "[Document Extract] Text docs:",
            ...
            console.log(
              "[Document Extract] Document details:",
              documentContents.map((d) => ({
                name: d.name,
                isText: d.isText,
                mimeType: d.mimeType,
                contentLen: d.content?.length || 0,
              })),
            );
            ...
            if (textDocuments.length > 0) {
              documentTextContent = textDocuments
                .map(
                  (d) => `
...
`,
                ...
                .join("\n");
              console.log(
                "[Document Extract] Text content preview:",
                documentTextContent.substring(0, 300),
              );
            }
            ...
            if (textDocuments.length > 0) {
              console.log(
                "[Document Extract] STEP 1: Processing TEXT documents with chunked extraction...",
              );
              ...
              for (let i = 0; i < documentTextContent.length; i += CHUNK_SIZE) {
                allChunks.push(
                  documentTextContent.substring(i, i + CHUNK_SIZE),
                );
              }

              console.log(
                `[Document Extract] Processing ${allChunks.length} chunks of ~${CHUNK_SIZE} chars each`,
              );
...
{"extractedFields": {"fieldName": {"value": "structured value with dates", "confidence": 85, "source": "document name"}}}`;
              ...
              for (let i = 0; i < allChunks.length; i++) {
                console.log(
                  `[Document Extract] Processing chunk ${i + 1}/${allChunks.length}`,
                );
                ...
                try {
                  ...
                  const chunkAiResponse =
                    chunkResponse.choices[0]?.message?.content || "";
                  const cleanedChunk = chunkAiResponse
                    .replace(/```json\n?/g, "")
                    .replace(/```\n?/g, "")
                    .trim();
                  ...
                  try {
                    ...
                    if (parsedChunk.extractedFields) {
                      ...
                      console.log(
                        `[Document Extract] Chunk ${i + 1} extracted ${Object.keys(parsedChunk.extractedFields).length} fields`,
                      );
                    }
                  } catch (parseErr) {
                    console.error(
                      `[Document Extract] Chunk ${i + 1} parse error:`,
                      parseErr,
                    );
                  }
                } catch (chunkError: any) {
                  console.error(
                    `[Document Extract] Chunk ${i + 1} failed:`,
                    chunkError.message,
                  );
              ...
              console.log(
                `[Document Extract] Merging ${chunkResults.length} chunk results`,
              );
              ...
              for (const chunkData of chunkResults) {
                for (const [field, data] of Object.entries(chunkData)) {
                  ...
                  if (
                    ...
                    (newConfidence === existingConfidence &&
                      valueStr.length > (extractedData[field]?.length || 0))
                  ) {
              ...
              console.log(
                "[Document Extract] Merged extraction complete, total fields:",
              ...
              for (const field of Object.keys(extractedData)) {
                ...
                if (
                  dateRangeFields.includes(field) &&
                  !value.match(/\d{4}/) &&
                  !value.includes("circa")
                ) {
            ...
            if (imageDocuments.length > 0) {
              console.log(
                "[Document Extract] STEP 2: Processing IMAGE documents with Vision API...",
              );
...
{"extractedFields": {"fieldName": {"value": "extracted value", "confidence": 90, "source": "passport"}}}`,
                ...
                try {
                  ...
                  if (visionParsed.extractedFields) {
                    ...
                    console.log(
                      "[Document Extract] Vision API extracted",
                      Object.keys(visionParsed.extractedFields).length,
                      "fields from images",
                    );
                  }
                } catch (visionParseErr) {
                  console.error(
                    "[Document Extract] Vision API parse error:",
                    visionParseErr,
                  );
                }
              } catch (visionError: any) {
                console.error(
                  "[Document Extract] Vision API error:",
                  visionError.message,
                );
              }
            }
            ...
            console.error(
              `[Document Extract] OpenAI failed: ${openaiError.message}`,
            );
          ...
          console.log(
            "[Document Extract] No document contents, using text-based extraction...",
          );
      ...
      if (Object.keys(extractedData).length === 0) {
        console.log(
          "[Document Extract] Using intelligent placeholders based on document metadata",
        );
      ...
      if (Object.keys(extractedData).length === 0) {
        console.log(
          "[Document Extract] Using generic placeholders as last resort",
        );
      ...
      console.log(
        "[Document Extract] Extracted fields:",
      ...
      try {
        const extraction = await db
          .insert(require("@shared/schema").documentExtractions)
          .values({
            userId: user.id,
      ...
      console.error(
        "[Document Extract] CRITICAL ERROR:",
        error?.message || error,
      );
    ...
    try {
      ...
      if (existingDemoPlans.length === 0) {
        const demoPlan = await storage.createBusinessPlan({
          ...
          existingCustomers:
            "Guy's and St Thomas' NHS Foundation Trust (18-month pilot, converting to full deployment), Royal Free London NHS Foundation Trust (12-month pilot), Barts Health NHS Trust (6-month pilot), Cambridge University Hospitals NHS Trust (active pilot). Private sector: HCA Healthcare UK (3 facilities), Nuffield Health (2 facilities), BMI Healthcare (pilot starting).",
          betaTesters:
      ...
      const documentCategories = [
        ...
        },
        {
          ...
          description:
            "Original passport showing travel history and previous visas",
          fileType: "application/pdf",
          fileSize: 3120000,
          status: "verified",
        ...
        },
        {
          ...
          description:
            "Business account statements showing £285,000 revenue and healthy cash flow",
          fileType: "application/pdf",
          fileSize: 1850000,
          status: "verified",
        },
        ...
        },
        {
          ...
          description:
            "Comprehensive 45-page business plan with financial projections",
          fileType: "application/pdf",
          fileSize: 8500000,
          status: "verified",
        },
        ...
        },
        {
          ...
          description:
            "Complete endorsement application with supporting evidence",
          fileType: "application/pdf",
          fileSize: 5600000,
          status: "pending",
        },
        ...
        },
        {
          ...
          description:
            "Professional reference from pilot programme stakeholder",
          fileType: "application/pdf",
          fileSize: 380000,
          status: "verified",
        },
        ...
        },
        {
          ...
          description:
            "Detailed Excel model with revenue projections and unit economics",
          fileType:
            "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
          fileSize: 1500000,
          status: "verified",
      ...
      res
        .status(500)
        .json({ error: "Failed to seed demo data", details: String(error) });
    }
  });
  ...
  app.get("/api/achievements", isAuthenticated, async (req, res) => {
    try {
      const user = req.user as any;
      const { getUserAchievements, getAllAchievements, getUserPoints } =
  ...
  app.get("/api/certificates", isAuthenticated, async (req, res) => {
    try {
      const user = req.user as any;
      const { getUserCertificates } = await import(
        "./services/achievementService"
      );
  ...
  app.post("/api/document-reviews", isAuthenticated, async (req, res) => {
    try {
      const user = req.user as any;
      const { createDocumentReview } = await import(
        "./services/documentReviewService"
      );
  ...
  app.get("/api/document-reviews", isAuthenticated, async (req, res) => {
    try {
      const user = req.user as any;
      const { getUserDocumentReviews, getDocumentReviewStats } = await import(
        "./services/documentReviewService"
      );
  ...
  app.get("/api/document-reviews/:id", isAuthenticated, async (req, res) => {
    try {
      const { getDocumentReview } = await import(
        "./services/documentReviewService"
      );
  ...
  app.post("/api/interview-sessions", isAuthenticated, async (req, res) => {
    try {
      const user = req.user as any;
      const { createInterviewSession } = await import(
  ...
  app.get("/api/interview-sessions", isAuthenticated, async (req, res) => {
    try {
      const user = req.user as any;
      const { getUserInterviewSessions, getInterviewStats } = await import(
        "./services/interviewService"
      );
  ...
  app.post(
    "/api/interview-sessions/:id/respond",
    isAuthenticated,
    async (req, res) => {
      try {
  ...
  app.get("/api/success-stories/:id", isAuthenticated, async (req, res) => {
    try {
      ...
      if (userTierIndex < storyTierIndex) {
        return res
          .status(403)
          .json({ error: "Upgrade required to access this story" });
      }
...
`;
      ...
      res.setHeader(
        "Content-Disposition",
        'attachment; filename="visa-journey-calendar.ics"',
      );
      res.send(icsContent);
    } catch (error) {
  ...
  app.get("/api/news", async (req, res) => {
    try {
      ...
      if (search && typeof search === "string") {
        ...
        news = await storage.getNewsByCategory(
          category,
        ...
        try {
          ...
          for (const article of data.articles) {
            ...
            if (
              ...
              titleLower.includes("endorsement")
            ) {
              category = "business";
            }
            ...
            if (
              titleLower.includes("innovator founder") ||
              titleLower.includes("uk visa")
            )
              relevanceScore += 30;
            if (titleLower.includes("immigration")) relevanceScore += 15;
            if (
              descLower.includes("endorsement") ||
              descLower.includes("tech nation")
            )
              relevanceScore += 20;
  ...
  const ACHIEVEMENTS = {
    ...
    streak_5: {
      id: "streak_5",
      title: "On Fire",
      description: "Answer 5 questions in a row",
      xp: 100,
      icon: "flame",
    },
    streak_10: {
      id: "streak_10",
      title: "Unstoppable",
      description: "Answer 10 questions in a row",
      xp: 200,
      icon: "zap",
    },
    quality_master: {
      ...
      description: "Score 90%+ on an answer",
      xp: 150,
      icon: "star",
    },
    section_complete: {
    ...
    agent_friend: {
      id: "agent_friend",
      title: "Agent Friend",
      description: "Complete questions with all 4 agents",
      xp: 300,
      icon: "users",
    },
    fifty_questions: {
    ...
    innovation_star: {
      ...
      description: "Score 80%+ on innovation section",
      xp: 400,
      icon: "lightbulb",
    },
    financial_wizard: {
      ...
      description: "Score 80%+ on viability section",
      xp: 400,
      icon: "trending-up",
    },
    growth_champion: {
      ...
      description: "Score 80%+ on scalability section",
      xp: 400,
      icon: "bar-chart",
    },
    compliance_pro: {
      ...
      description: "Score 80%+ on compliance section",
      xp: 400,
      icon: "shield",
    },
    perfect_answer: {
    ...
    detail_oriented: {
      ...
      description: "Write 500+ character answers 5 times",
      xp: 300,
      icon: "file-text",
    },
    visa_ready: {
  ...
  const LEVELS = [
    ...
    { level: 9, title: "Visa Pro", minXP: 15000, color: "#11b6e9" },
    {
      ...
      title: "Elite Founder",
      minXP: 25000,
      color: "linear-gradient(135deg, #ffa536, #11b6e9)",
    },
  ];
    ...
    for (let i = LEVELS.length - 1; i >= 0; i--) {
      if (xp >= LEVELS[i].minXP) {
        ...
        return {
          ...LEVELS[i],
          progress,
          nextLevelXP: nextLevel?.minXP || LEVELS[i].minXP,
        };
      }
    }
    ...
    try {
      ...
      res.json({
        ...
        agent: {
          ...
          greeting:
            "Hi! I'm Nova, your Innovation Specialist. I'll help you articulate what makes your business truly innovative. Let's explore your unique value proposition together!",
        },
      });
    } catch (error) {
  ...
  app.post(
    ...
    async (req, res) => {
      try {
        ...
        const agentKey = validAgents.includes(currentAgent)
          ? (currentAgent as keyof typeof allQuestions)
          : "nova";
        ...
        const questionsForCurrentAgent = answeredQuestionIds.filter(
          (id: string) => id.startsWith(currentAgent),
        ).length;
        const switchAgent =
          questionsForCurrentAgent >= switchThreshold &&
          questionsForCurrentAgent > 0;

        const agentOrder = ["nova", "sterling", "atlas", "sage"];
        const currentIndex = agentOrder.indexOf(currentAgent);
        const nextAgent = switchAgent
          ? agentOrder[(currentIndex + 1) % agentOrder.length]
          : currentAgent;
        ...
        const overallReadiness =
          (baseScores.innovationScore +
            baseScores.viabilityScore +
            baseScores.scalabilityScore +
            baseScores.complianceScore) /
          4;
        ...
        res.json({
          ...
          session: {
            ...
            level: getLevel(
              answeredQuestions * 50 + Math.floor(overallReadiness * 10),
            ),
          },
        });
      } catch (error) {
    ...
    async (req, res) => {
      try {
        ...
        const hasSpecificNames =
          /[A-Z][a-z]+\s[A-Z][a-z]+|Ltd|Inc|LLC|University|College/.test(
            answer,
          );
        ...
        if (
          totalQuestionsAnswered === 0 &&
          !achievements.includes("first_answer")
        ) {
        ...
        if (answerLength >= 500) {
          ...
          if (
            newDetailedCount >= 5 &&
            !achievements.includes("detail_oriented")
          ) {
        ...
        if (
          totalQuestionsAnswered + 1 === 50 &&
          !achievements.includes("fifty_questions")
        ) {
        ...
        if (qualityScore >= 90) {
          feedback =
            "Outstanding! This is exactly the kind of detailed, evidence-backed response that endorsers love. You've demonstrated clear expertise and provided concrete examples.";
        } else if (qualityScore >= 80) {
          feedback =
            "Excellent answer! You've provided specific details and evidence that endorsers look for. This strengthens your application significantly.";
        } else if (qualityScore >= 70) {
          feedback =
            "Very good! Your answer shows solid understanding. Adding a few more specific metrics or examples would make it even stronger.";
        } else if (qualityScore >= 60) {
          feedback =
            "Good answer! Consider adding more specific metrics, examples, or evidence to make it even more compelling for endorsers.";
        } else if (qualityScore >= 50) {
          feedback =
            "Decent start! Try to include specific numbers, dates, company names, or measurable outcomes to strengthen this response.";
        } else {
          feedback =
            "This is a starting point. Endorsers want to see specific details - numbers, dates, company names, market data, and measurable outcomes. Let's add more substance.";
        }
        ...
        const improvementSuggestions =
          qualityScore < 85
            ? [
                ...
                !hasCurrency &&
                  "Include financial figures (revenue, costs, investment)",
                !hasSpecificNames &&
                  "Mention specific company names, products, or people",
        ...
        const progressPercent =
          (newQuestionsAnswered / getTotalQuestionCount()) * 100;

        const baseScores = {
          innovationScore: Math.min(
            100,
            5 +
              newQuestionsAnswered * 0.5 +
              (qualityScore > 70 ? scoreChange : 0),
          ),
          viabilityScore: Math.min(
            100,
            3 +
              newQuestionsAnswered * 0.45 +
              (qualityScore > 70 ? scoreChange * 0.9 : 0),
          ),
          scalabilityScore: Math.min(
            100,
            4 +
              newQuestionsAnswered * 0.48 +
              (qualityScore > 70 ? scoreChange * 0.95 : 0),
          ),
          complianceScore: Math.min(
            100,
            2 +
              newQuestionsAnswered * 0.42 +
              (qualityScore > 70 ? scoreChange * 0.85 : 0),
          ),
        };
  ...
  app.post(
    "/api/ai-interview/enhance-answer",
    isAuthenticated,
    async (req, res) => {
      try {
        const user = req.user as any;
...
OUTPUT: Return ONLY the enhanced answer text, ready to submit. Do not include explanations or meta-commentary. Keep it between 100-250 words for optimal impact.`;
          ...
          messages: [
            {
              role: "system",
              content:
                "You are an expert visa application consultant specializing in UK Innovator Founder Visa. Enhance answers to be compelling, evidence-based, and endorser-ready.",
            },
  ...
  app.post(
    "/api/ai-interview/generate-draft",
    isAuthenticated,
    async (req, res) => {
      try {
        const user = req.user as any;
...
- Do not include explanations - just the draft answer ready to edit`;
          ...
          messages: [
            {
              role: "system",
              content:
                "You are an expert UK Innovator Founder Visa consultant. Generate helpful draft answers that applicants can personalize with their specific details.",
            },
  ...
  app.get(
    "/api/ai-interview/document-status",
    isAuthenticated,
    async (req, res) => {
      try {
        const user = req.user as any;
        ...
        for (const extraction of extractions) {
          if (
            ...
            extractedFieldsCount += Object.keys(
              extraction.extractedData,
            ).length;
          }
        }
  ...
  app.post(
    "/api/ai-interview/autofill-from-documents",
    isAuthenticated,
    async (req, res) => {
      try {
        const user = req.user as any;
        ...
        if (!premiumTiers.includes(userTier)) {
          return res.status(403).json({
            ...
            message:
              "Upgrade to Premium to autofill answers from your documents",
          });
        }
        ...
        if (!question) {
          return res.json({
            success: false,
            message:
              "No question detected. Please wait for the AI to ask a question first.",
          });
        }
        ...
        if (documents.length === 0) {
          return res.json({
            success: false,
            message:
              "No documents uploaded. Go to My Documents to upload your business documents first.",
            documentsCount: 0,
          });
        }
...
- Include any specific dates, numbers, or achievements found in documents`;
          ...
          messages: [
            {
              role: "system",
              content:
                "You are an expert UK Innovator Founder Visa consultant. Generate answers using the applicant's own document data to create personalized, evidence-based responses.",
            },
  ...
  app.post("/api/ai/oracle-delegate", isAuthenticated, requireToolAccess("oracle-supervisor"), async (req, res) => {
    try {
      ...
      const isQuestion =
        /^(what|when|how|why|where|who|which|can|do|does|is|are|should|would|could|will|has|have)\b/i.test(
          query.trim(),
        ) || query.includes("?");
...
Focus specifically on UK Innovator Founder Visa requirements and Home Office criteria.`;
      ...
      if (isQuestion) {
        ...
        let answer =
          "Based on UK Innovator Founder Visa requirements, I recommend consulting the official Home Office guidance for the most current information on your specific question.";

        if (
          queryLower.includes("time") ||
          queryLower.includes("long") ||
...
Reference actual visa requirements where relevant.`;
      ...
      const previousContext =
        previousSteps?.map((s: any) => `${s.id}: ${s.output}`).join("\n") || "";
      ...
      try {
        ...
        if (output) {
          console.log(
            `[Autopilot] OpenAI success for step: ${stepId}, output length: ${output.length}`,
          );
        }
      } catch (aiError: any) {
        console.error(
          `[Autopilot] OpenAI failed for step ${stepId}:`,
          aiError?.message,
        );
      }
      ...
      if (output) {
        ...
        console.warn(
          `[Autopilot] Both AI providers failed for step: ${stepId}, returning fallback`,
        );
        ...
        res.json({
          output:
            fallbacks[stepId] ||
            `${stepName} analysis completed. Your business shows potential for the UK Innovator Founder Visa. Review the guidance above and ensure you address each requirement thoroughly.`,
...
Keep responses focused, professional, and relevant to UK visa requirements.`;
      ...
      try {
        ...
        res.json({
          response:
            responseText ||
            "I would approach this by focusing on our unique value proposition and UK market opportunity.",
        });
      } catch (error: any) {
    ...
    try {
      ...
      if (!process.env.OPENAI_API_KEY) {
        return res.json({
          score: 0,
          feedback:
            "**Evaluation Unavailable.** AI service is not configured. Please contact support@ukvisaassistant.com for assistance.",
          error: true,
        });
      }
...
BE HARSH BUT FAIR. This is visa preparation - false confidence could lead to rejection.`;
      ...
      try {
        const feedbackText = await callAI(
        ...
        const scoreMatch =
          feedbackText.match(/\*?\*?Score:\s*(\d{1,3})\/100\*?\*?/i) ||
        ...
        return res.json({
          score: 0,
          feedback:
            "**Evaluation Unavailable.** We're experiencing a connectivity issue with our AI evaluation service. Please try again in a moment. If this problem persists, please contact support@ukvisaassistant.com for assistance.",
          error: true,
        });
      }
    } catch (error) {
  ...
  app.post("/api/ai/voice-to-document", isAuthenticated, requireToolAccess("voice-builder"), async (req, res) => {
    try {
      ...
      const template =
        documentTemplates[documentType] || documentTemplates["business-plan"];
  ...
  app.post("/api/admin/seed-test-users", requireAdmin, async (req, res) => {
    try {
      ...
      if (secretKey !== expectedKey) {
        return res
          .status(403)
          .json({ error: "Invalid seed key - operation not authorized" });
      }
      ...
      const existingTestUsers = await db
        .select()
        .from(users)
        .where(sql`email LIKE '%@ukvisatest.com'`)
        .limit(1);
        ...
        const matches = [
          {
            ...
            location: "London",
            matchScore: 92,
            expertise: ["Early-stage", "SaaS", "Fintech"],
            connectionReason:
              "Active investor in UK tech startups with focus on innovation visas",
          },
          {
            id: "n2",
            name: "Sarah Mitchell",
            ...
            location: "Manchester",
            matchScore: 88,
            expertise: ["Scaling", "Go-to-market", "UK market entry"],
            connectionReason:
              "Former founder with expertise in UK market expansion",
          },
          {
            id: "n3",
            name: "UK Innovation Partners",
            type: "partner",
            industry: industry || "Technology",
            location: "London",
            matchScore: 85,
            ...
            connectionReason:
              "Strategic partner for innovation-focused businesses",
          },
          {
            id: "n4",
            name: "David Chen",
            ...
            connectionReason:
              "Successfully navigated Innovator Founder visa process twice",
          },
          {
            id: "n5",
            name: "Seed Capital UK",
            type: "investor",
            industry: "Fintech",
            location: "Edinburgh",
            matchScore: 78,
            expertise: ["Seed stage", "SEIS/EIS", "Fintech"],
            connectionReason: "Focused on seed-stage UK fintech investments",
          },
        ...
        try {
          ...
          res.json({
            blueprint: {
              ...
              backgroundProblem:
                "Current solutions lack efficiency and fail to address the specific needs of users in this domain.",
              ...
              diagrams: [
                {
                  name: "Figure 1: System Architecture",
                  description:
                    "High-level overview of the system components and their interactions",
                },
                {
                  name: "Figure 2: Process Flow Diagram",
                  description: "Step-by-step flowchart of the main algorithm",
                },
                {
                  name: "Figure 3: Data Structure Diagram",
                  description:
                    "Representation of the key data structures used in the invention",
                },
              ],
            },
          });
    ...
    try {
      ...
      res.json({
        ...
        valuation: {
          ...
          methodology:
            "Comparable transaction analysis with UK market adjustment",
          factors: [
            {
              factor: "UK Market Entry",
              impact: "positive",
              description: "Strong UK market opportunity",
            },
            {
              factor: "Stage",
              impact: stage === "pre-seed" ? "negative" : "neutral",
              description: `${stage} stage risk profile`,
            },
            {
              factor: "Innovation Focus",
              impact: "positive",
              description: "Eligible for visa endorsement indicates innovation",
            },
            {
              factor: "Revenue",
              impact: "neutral",
              description: "Pre-revenue typical for stage",
            },
          ],
          comparables: [
            {
              company: "UK Fintech Seed 2024",
              valuation: "£3.5M",
              stage: "Seed",
            },
            {
              company: "London SaaS Pre-seed 2024",
              valuation: "£2M",
              stage: "Pre-seed",
            },
            {
              company: "Manchester Healthtech 2024",
              valuation: "£4M",
              stage: "Seed",
            },
          ],
...
- remediations: Array of 5 remediation strategy strings`;
      ...
      res
        .status(500)
        .json({ error: "Failed to generate remediation strategies" });
    }
  });
...
- recommendations: Array of improvement recommendations`;
          ...
          findings: [
            {
              ...
              issue: "Revenue projections lack supporting market data",
              severity: "high",
              suggestion: "Include third-party market research citations",
            },
            {
              category: "IP Protection",
              issue: "No patent applications mentioned",
              severity: "critical",
              suggestion:
                "File provisional patent applications for core innovations",
            },
            {
              category: "Scalability Plan",
              issue: "Job creation targets not clearly defined",
              severity: "medium",
              suggestion: "Specify hiring timeline with 2+ FTE by Year 3",
            },
          ],
          strengths: [
      ...
      try {
        ...
        if (verified && targetUser.email) {
          try {
            ...
            console.log(
              `Verification success email sent to ${targetUser.email}`,
            );
          } catch (emailError) {
  ...
  app.post(
    "/api/admin/users/:userId/tier-override",
    requireAdmin,
    async (req, res) => {
      try {
        const { userId } = req.params;
  ...
  app.post(
    ...
    async (req, res) => {
      try {
        ...
        const action =
          mode === "set" ? "Set" : amount >= 0 ? "Added" : "Removed";
        res.json({
          success: true,
          message:
            mode === "set"
  ...
  app.post(
    "/api/admin/restore-subscription",
    requireAdmin,
    async (req, res) => {
      try {
        const {
        ...
        console.log(
          `[ADMIN RESTORE] Admin ${admin.email} restored ${email} → ${tier} tier with ${creditsToGrant} credits`,
        );
  ...
  app.post("/api/admin/users/:userId/notes", requireAdmin, async (req, res) => {
    try {
      ...
      if (append && notes) {
        ...
        let existing: any[] = [];
        try {
          existing = JSON.parse(targetUser.adminNotes || "[]");
        } catch {
          existing = [];
        }
  ...
  app.get(
    "/api/admin/users/:userId/full-data",
    requireAdmin,
    async (req, res) => {
      try {
        const { userId } = req.params;
        ...
        const businessPlanCount = await db
  ...
  app.get(
    "/api/admin/users/:userId/impersonate-data",
    requireAdmin,
    async (req, res) => {
      try {
        const { userId } = req.params;
        ...
        const toolUsageRaw = await safeQ(
          () =>
            db.execute(sql`
      ...
      `),
        ...
        const toolProgressRaw = await safeQ(
          () =>
            db.execute(sql`
      ...
      `),
        ...
        const pageViewsRaw = await safeQ(
          () =>
            db.execute(sql`
      ...
      `),
        ...
        const toolRows =
          (toolUsageRaw.rows?.length
            ? toolUsageRaw.rows
            : toolProgressRaw.rows) || [];
        ...
        res.json({
          ...
          impersonationNote:
            "Read-only support view — no actions performed on behalf of user",
        });
      } catch (error) {
  ...
  app.delete("/api/admin/users/:userId", requireAdmin, async (req, res) => {
    try {
      ...
      await db.execute(
        sql.raw(
          `DELETE FROM page_views WHERE session_id IN (SELECT id FROM user_sessions WHERE user_id = '${userId}')`,
        ),
      );
      await db.execute(
        sql.raw(
          `DELETE FROM activity_events WHERE session_id IN (SELECT id FROM user_sessions WHERE user_id = '${userId}')`,
        ),
      );
      ...
      for (const row of fkResult.rows as any[]) {
        ...
        if (REASSIGN_TO_ADMIN[child_table]?.includes(child_column)) {
          try {
            await db.execute(
              sql.raw(
                `UPDATE "${child_table}" SET "${child_column}" = '${adminUser.id}' WHERE "${child_column}" = '${userId}'`,
              ),
            );
          } catch {
            /* ignore */
        ...
        if (nullable === "YES") {
          ...
          try {
            await db.execute(
              sql.raw(
                `UPDATE "${child_table}" SET "${child_column}" = NULL WHERE "${child_column}" = '${userId}'`,
              ),
            );
          } catch {
            /* ignore */
          ...
          try {
            await db.execute(
              sql.raw(
                `DELETE FROM "${child_table}" WHERE "${child_column}" = '${userId}'`,
              ),
            );
          } catch {
            /* ignore */
      ...
      try {
        await db.execute(
          sql.raw(`DELETE FROM user_sessions WHERE user_id = '${userId}'`),
        );
      } catch {
        /* ignore */
      }
  ...
  app.get(
    "/api/admin/users/:userId/activity-details",
    requireAdmin,
    async (req, res) => {
      try {
        const { userId } = req.params;
      ...
      `),
        ...
        const activityLogsRaw = await safeQ(
          () =>
            db.execute(sql`
      ...
      `),
        ...
        const toolProgressTopRaw = await safeQ(
          () =>
            db.execute(sql`
        SELECT tool_id,
               NULL::text as tool_category,
  ...
  app.get(
    "/api/admin/users/:userId/analysis",
    requireAdmin,
    async (req, res) => {
      try {
        const { userId } = req.params;
        ...
        const totalAiInteractions = await safeQuery(async () => {
          const r = await db.execute(
            sql`SELECT COUNT(*) as total FROM ai_action_logs WHERE user_id = ${userId}`,
          );
          return (r as any).rows?.[0]?.total || 0;
        }, 0);

        // Interview sessions
        const interviewStats = await safeQuery(
          async () => {
            const r = await db.execute(sql`
        ...
        `);
            return (
              (r as any).rows?.[0] || { total_spent: 0, transaction_count: 0 }
            );
          },
        ...
        const pageViewCount = await safeQuery(async () => {
          const r = await db.execute(
            sql`SELECT COUNT(*) as cnt FROM page_views WHERE user_id = ${userId}`,
          );
        ...
        const sessionCount = await safeQuery(async () => {
          const r = await db.execute(
            sql`SELECT COUNT(*) as cnt FROM user_sessions WHERE user_id = ${userId}`,
          );
          return parseInt(String((r as any).rows?.[0]?.cnt)) || 0;
        }, 0);
        ...
        const activityEventsCount = await safeQuery(async () => {
          const r = await db.execute(
            sql`SELECT COUNT(*) as cnt FROM activity_events WHERE user_id = ${userId}`,
          );
          return parseInt(String((r as any).rows?.[0]?.cnt)) || 0;
        }, 0);
        ...
        // Uploaded files
        const fileStats = await safeQuery(
          async () => {
            const r = await db.execute(sql`
        ...
        const engagementScore = Math.round(
          Math.min(
            100,
            tierScore + planScore + toolScore + pvScore + sessScore + evtScore,
          ),
        );
        ...
        const analysis = {
          ...
          credits: {
            ...
            transactions:
              parseInt(String((creditStats as any).total_transactions)) || 0,
          },
          financials: {
            totalSpent: parseFloat(String((payments as any).total_spent)) || 0,
            transactionCount:
              parseInt(String((payments as any).transaction_count)) || 0,
            firstPayment: (payments as any).first_payment || null,
            lastPayment: (payments as any).last_payment || null,
            lifetimeValue:
              parseFloat(String((payments as any).total_spent)) || 0,
          },
          businessPlans: {
          ...
          toolEngagement: {
            ...
            totalToolInteractions: allToolUsage.reduce(
              (sum: number, t: any) => sum + parseInt(String(t.uses)),
              0,
            ),
          },
          aiInteractions: {
            ...
            byType: aiLogs,
            interviewSessions:
              parseInt(String((interviewStats as any).total)) || 0,
            completedInterviews:
              parseInt(String((interviewStats as any).completed)) || 0,
          },
          support: {
            totalTickets: parseInt(String((tickets as any).total)) || 0,
            openTickets: parseInt(String((tickets as any).open_count)) || 0,
            resolvedTickets:
              parseInt(String((tickets as any).resolved_count)) || 0,
          },
          security: {
            events: securityEvents,
            totalEvents: securityEvents.reduce(
              (sum: number, e: any) => sum + parseInt(String(e.count)),
              0,
            ),
          },
          feedback: {
          ...
          referrals: {
            codesCreated:
              parseInt(String((referralStats as any).codes_created)) || 0,
            successfulReferrals:
              parseInt(String((referralStats as any).successful_referrals)) ||
              0,
            totalRewards:
              parseFloat(String((referralStats as any).total_rewards)) || 0,
          },
          files: {
          ...
          eligibility: {
            ...
            latestScore:
              eligibilityAssessments.length > 0
                ? (eligibilityAssessments[0] as any).score
                : null,
          },
          insights: {
            engagementScore,
            ...
            churnRisk:
              riskLevel === "high"
                ? "High risk — inactive for 30+ days"
                : riskLevel === "medium"
                  ? "Medium risk — inactive for 14–30 days"
                  : "Low risk — recently active",
            upgradeReadiness:
              userData.subscriptionTier === "free" && engagementScore > 50
            ...
            recommendedActions: [
              ...(riskLevel === "high"
                ...
                : []),
              ...(userData.subscriptionTier === "free" && engagementScore > 60
                ? ["Offer upgrade discount"]
                : []),
              ...(!userData.isEmailVerified
                ? ["Chase email verification"]
                : []),
              ...(parseInt(String((tickets as any).open_count)) > 0
                ? ["Resolve open support tickets"]
                : []),
              ...(userPlans.length === 0
                ? ["Encourage creation of first business plan"]
                : []),
              ...(allToolUsage.length === 0
                ? ["Send tool onboarding guide"]
                : []),
            ],
          },
      ...
      const updates = [
        {
          ...
          summary:
            "Home Office has clarified the endorsement criteria for technology startups.",
          impact: "high",
        ...
        },
        {
          ...
          summary:
            "Corporation tax rate is 25% for companies with profits over £250,000.",
          impact: "medium",
          date: new Date().toISOString().split("T")[0],
          category: "tax",
          source: "HMRC",
  ...
  app.get("/api/admin/feedback", requireAdmin, async (req, res) => {
    try {
      ...
      const siteFeedbackRows = await db
        .select()
        .from(siteFeedback)
        .orderBy(sql`created_at DESC`)
        .limit(200);
  ...
  app.post("/api/feedback/floating", async (req, res) => {
    try {
      ...
      console.log(
        `[Floating Feedback] New ${type} feedback${ratingVal ? ` (${ratingVal}★)` : ""} from ${email}`,
      );
  ...
  app.patch(
    "/api/admin/floating-feedback/:id",
    requireAdmin,
    async (req, res) => {
      try {
        const { id } = req.params;
  ...
  app.post(
    ...
    async (req, res) => {
      try {
        ...
        const recipients = usersResult.rows as Array<{
          id: string;
          email: string;
              ...
              batch.map((user) => {
                ...
                const messageHtml = containsHtml
                  ...
                  : cleanMsg
                      ...
                      .map(
                        (para: string) =>
                          `<p style="margin:0 0 14px;color:#374151;font-size:15px;line-height:1.7;">${para.replace(/\n/g, "<br>")}</p>`,
                      )
                      .join("");
                return sendEmail({
                  to: user.email,
...
<body style="margin:0;padding:0;background-color:#f3f4f6;font-family:Arial,Helvetica,sans-serif;">
    ...
    <tr><td align="center">
      <table role="presentation" width="600" cellpadding="0" cellspacing="0" style="max-width:600px;width:100%;background-color:#ffffff;border-radius:8px;overflow:hidden;">
        ...
        <tr>
          ...
          console.log(
            `[Broadcast] Sent notification "${notification.title}" to ${recipientCount} user(s)`,
          );
        }
      } catch (error) {
  ...
  app.post(
    "/api/admin/social-content/generate",
    requireAdmin,
    async (req, res) => {
      try {
        ...
        const styleDescriptions: Record<string, string> = {
          "linkedin-conversion":
            "a high-converting LinkedIn post with a strong hook, pain point, solution reveal, and clear CTA. Use short punchy lines, white space, and a signature 'save this post' or 'share with someone who needs this' CTA.",
          "linkedin-story":
            "a LinkedIn story/case study post following the arc: before-struggle → discovery of the platform → transformation → result → takeaway. Make it emotionally resonant and specific.",
          "linkedin-engagement":
            "a LinkedIn engagement post that sparks debate or opinion. Use a bold statement or question, invite people to comment, and create conversation. No hard CTA — just engagement.",
          "linkedin-value":
            "a LinkedIn value post that teaches the reader something specific about the UK Innovator Founder Visa route. Use a numbered list or short paragraphs. End with a soft CTA to try the platform.",
          "twitter-thread":
            "a Twitter/X thread of 6-9 tweets. Start with a strong hook tweet (tweet 1), then deliver value in subsequent tweets, and end with a CTA tweet. Format each tweet as '1/', '2/' etc.",
        };

        const voiceDescriptions: Record<string, string> = {
          platform:
            "Write from the perspective of the Innovator Founder Visa Assistant platform brand — professional, authoritative, informative, empowering. We are a UK-based AI tool that helps international founders navigate the UK Innovator Founder Visa route.",
          founder:
            "Write from the perspective of a founder who built this platform — personal, builder mindset, behind-the-scenes, relatable. Use 'I' voice, share the journey of building the tool.",
          student:
            "Write from the perspective of an international student or graduate who used the platform to navigate the UK Innovator Founder Visa — relatable, authentic, slightly anxious-turned-confident tone.",
        };
        ...
        const MIN_SOCIAL_WORDS = 180;
        const countWords = (text: string) =>
          text.trim().split(/\s+/).filter(Boolean).length;
        const isLongEnough = (text: string) =>
          countWords(text) >= MIN_SOCIAL_WORDS;
...
- The post must be complete and substantial.
- For LinkedIn posts, write 180-280 words.
- For Twitter/X threads, write 6-9 full tweets with useful detail in each tweet.
- Do not stop after the hook. Include the hook, problem, insight, solution, and CTA.
...
- Output ONLY the post itself — no preamble, no explanation, no title, no markdown formatting headers. Just the raw post text ready to copy-paste.`;
  ...
  app.delete(
    "/api/admin/performance/cleanup",
    requireAdmin,
    async (req, res) => {
      try {
        const thirtyDaysAgo = new Date(Date.now() - 30 * 24 * 60 * 60 * 1000);
        const deleted =
          await storage.cleanupOldPerformanceMetrics(thirtyDaysAgo);
        res.json({
          deleted,
  ...
  app.post("/api/activity/session", isAuthenticated, async (req, res) => {
    try {
      ...
      await db.execute(
        sql`UPDATE users SET last_activity_at = NOW() WHERE id = ${userId}`,
      );
  ...
  app.post("/api/activity/page-view", isAuthenticated, async (req, res) => {
    try {
      ...
      if (!sessionId || !pagePath) {
        return res
          .status(400)
          .json({ error: "Session ID and page path required" });
      }

      // Round pageLoadTimeMs to integer for database
      const roundedLoadTime = pageLoadTimeMs
        ? Math.round(pageLoadTimeMs)
        : null;
      ...
      res.json({
        realtime: {
          ...
          avgSessionDuration: Math.round(
            parseFloat(avgDuration.rows[0]?.avg_seconds || "0"),
          ),
        },
  ...
  app.get(
    "/api/admin/activity/journey/:userId",
    requireAdmin,
    async (req, res) => {
      try {
        const { userId } = req.params;
        ...
        res.json({
          ...
          stats: {
            ...
            avgSessionDuration: Math.round(
              parseFloat(avgSessionDuration.rows[0]?.avg_seconds || "0"),
            ),
          },
  ...
  app.post("/api/activity/session", async (req, res) => {
    try {
      ...
      const { sessionToken, deviceInfo, location, referrer, utm, currentPage } =
        req.body;
  ...
  app.get("/api/blog/cover", (req, res) => {
    ...
    const palettes: Record<
      string,
      { bg1: string; bg2: string; accent: string; tag: string }
    > = {
      "visa-updates": {
        bg1: "#003087",
        bg2: "#005EB8",
          ...
          const [inserted] = await db
            ...
            .values({
              ...
              verificationExpiresAt:
                (post as any).verificationExpiresAt ?? null,
  ...
  const handleCronBlogGeneration = async (req: any, res: any) => {
    try {
      ...
      const expectedSecret =
        process.env.CRON_SECRET || "ukivfa-cron-secret-2026-daily-posts";
      ...
      setImmediate(async () => {
        try {
          ...
          for (const post of posts) {
            try {
              await db.insert(blogPosts).values({
                ...
                openaiScore: (post as any).openaiScore ?? null,
                verificationStatus:
                  (post as any).verificationStatus ?? "pending",
                verificationDetails: (post as any).verificationDetails ?? null,
                verifiedAt: (post as any).verifiedAt ?? null,
                verificationExpiresAt:
                  (post as any).verificationExpiresAt ?? null,
          ...
          console.log(
            `[Cron] Background generation complete: ${insertedCount} blog posts created`,
          );
        } catch (bgError) {
  ...
  app.post("/api/admin/blog/fix-images", requireAdmin, async (req, res) => {
    try {
      ...
      const isGoodUrl = (url: string | null) =>
        !!url && url.startsWith("/objects/blog/");
  ...
  app.post("/api/admin/blog/seed", requireAdmin, async (req, res) => {
    try {
      ...
      console.log(
        `[Admin] Starting bulk blog generation: ${count} posts, ${postsPerDay}/day, starting ${daysAgo} days ago`,
      );
      ...
      setImmediate(async () => {
        try {
          const posts = await generateBackdatedPosts(
            count,
            postsPerDay,
            daysAgo,
          );
          ...
          for (const post of posts) {
            try {
              ...
              insertedCount++;
              console.log(
                `[Admin] Inserted backdated post ${insertedCount}: ${post.title}`,
              );
            } catch (insertError: any) {
              console.error(
                "Failed to insert backdated post:",
          ...
          console.log(
            `[Admin] Bulk blog seeding complete: ${insertedCount} posts created`,
          );
        } catch (bgError) {
  ...
  app.get(
    "/api/admin/analytics/comprehensive-kpis",
    requireAdmin,
    async (req, res) => {
      try {
        const now = new Date();
  ...
  app.get(
    "/api/admin/analytics/api-performance",
    requireAdmin,
    async (req, res) => {
      try {
  ...
  app.post("/api/analytics/track", async (req, res) => {
    try {
      const user = req.user as any;
      const { eventName, properties, pagePath, sessionId, deviceInfo } =
        req.body;
  ...
  app.post(
    "/api/admin/analytics/log-latency",
    requireAdmin,
    async (req, res) => {
      try {
        const {
  ...
  app.get(
    "/api/admin/analytics/tool-performance",
    requireAdmin,
    async (req, res) => {
      try {
        const days = parseInt(req.query.days as string) || 7;
  ...
  // Export analytics
  app.get(
    "/api/admin/analytics/export-performance",
    requireAdmin,
    async (req, res) => {
      try {
        const days = parseInt(req.query.days as string) || 7;
        ...
        `);
          ...
          /* export_analytics table not yet created in this env */
        }
  ...
  app.get(
    "/api/admin/analytics/coins-usage",
    requireAdmin,
    async (req, res) => {
      try {
        const days = parseInt(req.query.days as string) || 30;
  ...
  app.post(
    "/api/admin/analytics/aggregate-hourly",
    requireAdmin,
    async (req, res) => {
      try {
        const now = new Date();
  ...
  app.post("/api/admin/blog/deduplicate", requireAdmin, async (req, res) => {
    try {
      ...
      console.log(
        `[Admin] Blog deduplication: removed ${toDelete.length} duplicates, kept ${kept.length} posts`,
      );
    ...
    try {
      ...
      if (generatedPost) {
        ...
        const needsHumanReview =
          (generatedPost as any).verificationStatus === "human_review";
        ...
        const [savedPost] = await db
          ...
          .values({
            ...
            aiVerificationScore:
              (generatedPost as any).aiVerificationScore ?? null,
            ...
            verificationStatus:
              (generatedPost as any).verificationStatus ?? "pending",
            verificationDetails:
              (generatedPost as any).verificationDetails ?? null,
            verifiedAt: (generatedPost as any).verifiedAt ?? null,
            verificationExpiresAt:
              (generatedPost as any).verificationExpiresAt ?? null,
            humanReviewRequired:
              (generatedPost as any).humanReviewRequired ?? false,
        ...
        console.log(
          `[Blog Generator] Post saved to DB: id=${savedPost.id}, status=${postStatus}, verificationStatus=${savedPost.verificationStatus}`,
        );
  ...
  app.post(
    "/api/admin/blog/posts/:postId/reverify",
    requireAdmin,
    async (req, res) => {
      try {
        const { postId } = req.params;
        const { verifyBlogPost, computeContentHash } = await import(
          "./blogMultiVerifier"
        );
  ...
  app.post(
    "/api/admin/blog/posts/:postId/auto-fix",
    requireAdmin,
    async (req, res) => {
      try {
        const { postId } = req.params;
        ...
        await db
          ...
          .set({
            ...
            verificationStatus: verificationResult.passed
              ? "passed"
              : "human_review",
  ...
  app.get("/api/admin/blog/verifier-status", requireAdmin, async (req, res) => {
    ...
    const statuses = [
      {
        name: "Gemini",
        status: result.geminiScore !== null ? "ok" : "unavailable",
        score: result.geminiScore,
      },
      {
        name: "OpenAI",
        status: result.openaiScore !== null ? "ok" : "unavailable",
        score: result.openaiScore,
      },
      {
        name: "Claude",
  ...
  app.post(
    "/api/admin/blog/posts/:postId/approve",
    requireAdmin,
    async (req, res) => {
      try {
        const { postId } = req.params;
  ...
  app.post("/api/seo/strategy", requireAdmin, async (req, res) => {
    try {
      ...
      if (!ctx.businessName || !ctx.primaryService) {
        return res
          .status(400)
          .json({ error: "businessName and primaryService are required" });
      }
  ...
  app.post("/api/seo/activate-automation", requireAdmin, async (req, res) => {
    try {
      ...
      if (!strategy || !strategy.businessContext) {
        return res
          .status(400)
          .json({ error: "Full strategy result is required" });
      }
  ...
  app.post("/api/seo/automation-toggle", requireAdmin, async (req, res) => {
    try {
      const { planId, action } = req.body as {
        planId: string;
        action: "pause" | "resume";
      };
...
<div class="page">
  ...
  app.get(
    "/api/admin/support/user-logs-export",
    requireAdmin,
    async (req, res) => {
      const { email } = req.query as { email: string };
      if (!email) return res.status(400).json({ error: "email is required" });
      ...
      try {
        ...
        if (!user) {
          return res
            .status(404)
            .send(
              `<html><body style="font-family:Arial;padding:40px;"><h2>User not found</h2><p>No user found with email: <strong>${email}</strong></p><p>Note: This searches the <em>current</em> database. If you need production data, run this on Railway.</p></body></html>`,
            );
        }
        ...
        const totalDurationMins = Math.round(
          sessions.reduce((a, s) => a + (s.totalDurationSeconds || 0), 0) / 60,
        );
        ...
        pages.forEach((p) => {
          pageFreq[p.pagePath] = (pageFreq[p.pagePath] || 0) + 1;
        });
        ...
        toolEvents.forEach((e) => {
          if (e.toolId) toolFreq[e.toolId] = (toolFreq[e.toolId] || 0) + 1;
        });
...
<div class="section">
  ...
  ${
    stripePayments.length === 0
      ? `<p style="color:#6b7280">No Stripe payment records found. Stripe customer ID: ${stripeCustomerId || "not linked"}.</p>`
      : `
  <table>
...
<div class="section page-break">
  ...
  ${
    sessions.length === 0
      ? '<p style="color:#6b7280">No sessions recorded in this database.</p>'
      : `
  <table>
...
<div class="section">
  ...
  ${
    topPages.length === 0
      ? '<p style="color:#6b7280">No page view data recorded.</p>'
      : `
  <table>
...
<div class="section">
  ...
  ${
    topTools.length === 0
      ? '<p style="color:#6b7280">No tool usage recorded.</p>'
      : `
  ...
  <table>
    ...
    ${topTools
      .map(
        ([tool, count]) => `
    <tr><td>${tool}</td><td><strong>${count}</strong></td></tr>`,
      )
      .join("")}
  </table>`
  }
...
<div class="section page-break">
  ...
  ${
    events.length === 0
      ? '<p style="color:#6b7280">No activity events recorded.</p>'
      : `
  <table>
...
<div class="section">
  ...
  ${
    plans.length === 0
      ? '<p style="color:#6b7280">No business plans created.</p>'
      : `
...
<div class="section">
  ...
  ${
    credits.length === 0
      ? '<p style="color:#6b7280">No credit transactions recorded.</p>'
      : `
...
<div class="section">
  ...
  ${
    interviews.length === 0
      ? '<p style="color:#6b7280">No AI interview sessions recorded.</p>'
      : `
...
<div class="section">
  ...
  ${
    auditActions.length === 0
      ? '<p style="color:#6b7280">No admin actions recorded against this account.</p>'
      : `
  <table>
...
<div class="footer">
  Full Activity Log Export — ${user.email} — Generated: ${today} — UK Innovator Founder Visa Assistant — CONFIDENTIAL
        ...
        res
          .status(500)
          .send(
            `<html><body style="font-family:Arial;padding:40px;"><h2>Error generating log</h2><pre>${error.message}</pre></body></html>`,
          );
      }
    },
  );
  ...
  app.get("/api/seo/backlink-targets", requireAdmin, async (_req, res) => {
    try {
      const targets = await db
        ...
        .orderBy(desc(backlinkTargets.createdAt));
      res.json(targets);
    } catch (e: any) {
  ...
  app.patch("/api/seo/backlink-targets/:id", requireAdmin, async (req, res) => {
    try {
      ...
      const [updated] = await db
        ...
        .set(updates)
        .where(eq(backlinkTargets.id, id))
        .returning();
      res.json(updated);
    } catch (e: any) {
      res.status(500).json({ error: e.message });
...
Return ONLY the JSON array. No markdown. No explanation.`;
      ...
      const existing = await db
        .select({ url: backlinkTargets.url })
        .from(backlinkTargets);
      const existingUrls = new Set(existing.map((e) => e.url));
      const newTargets = discovered.filter(
        (t) => t.url && !existingUrls.has(t.url),
      );

      if (newTargets.length > 0) {
        await db.insert(backlinkTargets).values(
          newTargets.map((t) => ({
            name: t.name || "Unknown",
            url: t.url,
      ...
      res.json({
        ...
        skipped: discovered.length - newTargets.length,
      });
    } catch (e: any) {
