...
// Blog Posts table for automated SEO content with comprehensive scheduling & analytics
export const blogPosts = pgTable("blog_posts", {
  ...
  // Scheduling System
  postStatus: varchar("post_status", { length: 20 }).notNull().default('published'), // draft, scheduled, published, archived
  scheduledFor: timestamp("scheduled_for"), // When to auto-publish
  generatedAt: timestamp("generated_at"), // When AI generated this post
  isAutoGenerated: boolean("is_auto_generated").notNull().default(false),
  wasEdited: boolean("was_edited").notNull().default(false), // If manually edited after generation
  editedAt: timestamp("edited_at"),
  editedBy: varchar("edited_by"), // Admin user who edited
  originalContent: text("original_content"), // Backup of AI-generated content before edits
  
  // Analytics & Engagement
  likes: integer("likes").notNull().default(0),
  shares: integer("shares").notNull().default(0),
  comments: integer("comments").notNull().default(0),
  avgTimeOnPage: integer("avg_time_on_page").notNull().default(0), // seconds
  bounceRate: real("bounce_rate").notNull().default(0), // percentage
  socialShares: jsonb("social_shares"), // {"twitter": 10, "linkedin": 5, "facebook": 3}
  
  // SEO Performance
  googleRanking: integer("google_ranking"), // Position in Google search
...
// Blog Generation Queue for scheduled auto-generation
export const blogGenerationQueue = pgTable("blog_generation_queue", {
  id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
  
  targetDate: timestamp("target_date").notNull(), // The date this post is for
  topic: text("topic"), // Pre-selected topic if any
  category: varchar("category", { length: 100 }),
  
  status: varchar("status", { length: 20 }).notNull().default('pending'), // pending, generating, generated, failed, published
  generatedPostId: varchar("generated_post_id"), // Reference to blogPosts.id when generated
  ...
  index("idx_blog_queue_date").on(table.targetDate),
  index("idx_blog_queue_status").on(table.status),
]);

export const insertBlogGenerationQueueSchema = createInsertSchema(blogGenerationQueue).omit({
  id: true,
  createdAt: true,
});

export type BlogGenerationQueue = typeof blogGenerationQueue.$inferSelect;
